...
function hide(){stopPromoCarousel();document.querySelectorAll(".view").forEach(x=>x.classList.add("hidden"))}
function fillAreas(){
...
let promoBannerCache=[];
let promoAdminRows=[];
let promoSlideIndex=0;
let promoSlideTimer=null;
let promoCleanupDone=false;
...
function todayIsoDate(){return todayLocalKey()}
function addDaysIso(base,days){const d=new Date(`${base}T12:00:00`);d.setDate(d.getDate()+days);return d.toLocaleDateString('en-CA')}
function formatPromoDate(value){if(!value)return '-';const d=new Date(`${value}T12:00:00`);return d.toLocaleDateString('id-ID',{day:'2-digit',month:'short',year:'numeric'})}

async function loadPromoBanners({force=false}={}){
 if(promoBannerCache.length&&!force)return promoBannerCache;
 const auth=getSbSession();
 if(!auth?.session_token){promoBannerCache=[];return promoBannerCache}
 try{const rows=await rpc("app_get_active_promo_banners",{p_token:auth.session_token});promoBannerCache=Array.isArray(rows)?rows:[];}
 catch(e){console.warn("Gagal memuat promo",e);promoBannerCache=[]}
 return promoBannerCache;
}
async function loadAdminPromoBanners(){
 const auth=getSbSession();
 if(currentUser?.role!=="admin"||!auth?.session_token){promoAdminRows=[];return promoAdminRows}
 try{const rows=await rpc("app_admin_list_promo_banners",{p_token:auth.session_token});promoAdminRows=Array.isArray(rows)?rows:[]}
 catch(e){console.warn("Gagal memuat daftar banner",e);promoAdminRows=[];toast(`Gagal memuat banner: ${e.message||e}`)}
 return promoAdminRows;
}
function renderSalesPromoCarousel(){
 const card=document.getElementById("salesPromoCard");
 if(!card)return;
 const rows=promoBannerCache||[];
 if(currentUser?.role==="admin"||!rows.length){card.classList.add("hidden");stopPromoCarousel();return}
 card.classList.remove("hidden");
 promoSlideIndex=Math.max(0,Math.min(promoSlideIndex,rows.length-1));
 const row=rows[promoSlideIndex];
 const title=document.getElementById("salesPromoTitle");if(title)title.textContent=row?.title||"Promo Terbaru";
 setPromoImage(document.getElementById("salesPromoImage"),null,row?.image_data||"");
 const dots=document.getElementById("promoDots");
 if(dots)dots.innerHTML=rows.length>1?rows.map((_,i)=>`<button class="promo-dot ${i===promoSlideIndex?'active':''}" onclick="goToPromoSlide(${i})" aria-label="Promo ${i+1}"></button>`).join(''):'';
 document.getElementById("promoPrevBtn")?.classList.toggle("hidden",rows.length<2);
 document.getElementById("promoNextBtn")?.classList.toggle("hidden",rows.length<2);
 if(rows.length>1)startPromoCarousel();else stopPromoCarousel();
}
function changePromoSlide(step){const total=promoBannerCache.length;if(total<2)return;promoSlideIndex=(promoSlideIndex+step+total)%total;renderSalesPromoCarousel()}
function goToPromoSlide(index){promoSlideIndex=index;renderSalesPromoCarousel()}
function startPromoCarousel(){stopPromoCarousel();promoSlideTimer=setInterval(()=>changePromoSlide(1),5000)}
function stopPromoCarousel(){if(promoSlideTimer){clearInterval(promoSlideTimer);promoSlideTimer=null}}

function renderAdminPromoGallery(){
 const box=document.getElementById("adminPromoGallery"),count=document.getElementById("promoBannerCount");
 if(count)count.textContent=`${promoAdminRows.length} banner`;
 if(!box)return;
 if(!promoAdminRows.length){box.innerHTML='<div class="empty">Belum ada banner.</div>';return}
 const today=todayIsoDate();
 box.innerHTML=promoAdminRows.map(row=>{
  const periodActive=row.is_active&&row.start_date<=today&&row.end_date>=today;
  const status=!row.is_active?'Nonaktif':row.end_date<today?'Berakhir':row.start_date>today?'Terjadwal':'Aktif';
  return `<article class="promo-gallery-item">
   <img src="${row.image_data}" alt="${esc(row.title||'Promo')}" loading="lazy">
   <div class="promo-gallery-body"><div class="promo-gallery-title"><strong>${esc(row.title||'Promo')}</strong><span class="badge ${periodActive?'success':''}">${status}</span></div>
   <small>${formatPromoDate(row.start_date)} – ${formatPromoDate(row.end_date)} · Urutan ${Number(row.sort_order)||0}</small>
   <div class="promo-gallery-actions"><button class="secondary compact" onclick="editPromoBanner('${row.id}')">Edit</button><button class="danger-soft compact" onclick="deletePromoBanner('${row.id}')">Hapus</button></div></div>
  </article>`
 }).join('')
}
async function refreshPromoPanels(){
 const isAdmin=currentUser?.role==="admin";
 document.getElementById("adminPromoCard")?.classList.toggle("hidden",!isAdmin);
 if(isAdmin){
  if(!promoCleanupDone){promoCleanupDone=true;try{const auth=getSbSession();if(auth?.session_token)await rpc("app_admin_cleanup_expired_promo_banners",{p_token:auth.session_token})}catch(e){console.warn('Cleanup promo gagal',e)}}
  await loadAdminPromoBanners();renderAdminPromoGallery();
  document.getElementById("salesPromoCard")?.classList.add("hidden");
  if(!document.getElementById('promoEditId')?.value)resetPromoForm(false);
 }else{
  await loadPromoBanners({force:true});renderSalesPromoCarousel();
 }
}
function dataUrlBytes(dataUrl){const base64=(dataUrl.split(',')[1]||'');return Math.ceil(base64.length*3/4)}
function readPromoFileAsCompressedDataUrl(file){
 return new Promise((resolve,reject)=>{
  ...
  reader.onload=()=>{const img=new Image();img.onerror=()=>reject(new Error("Gambar tidak dapat dibuka"));img.onload=()=>{
   const maxW=1200,maxH=600,scale=Math.min(1,maxW/img.width,maxH/img.height);const canvas=document.createElement("canvas");canvas.width=Math.max(1,Math.round(img.width*scale));canvas.height=Math.max(1,Math.round(img.height*scale));canvas.getContext("2d").drawImage(img,0,0,canvas.width,canvas.height);
   let quality=.82,data=canvas.toDataURL("image/jpeg",quality);while(dataUrlBytes(data)>320000&&quality>.42){quality-=.08;data=canvas.toDataURL("image/jpeg",quality)}resolve(data)
  };img.src=reader.result};reader.readAsDataURL(file)
 })
}
async function previewPromoImageInput(){
 const file=document.getElementById("promoImageInput")?.files?.[0];if(!file)return;
 try{const data=await readPromoFileAsCompressedDataUrl(file);setPromoImage(document.getElementById("adminPromoPreview"),document.getElementById("adminPromoEmpty"),data);document.getElementById("adminPromoPreview").dataset.pending=data}
 catch(e){toast(e.message||'Gagal membaca gambar')}
}
function resetPromoForm(clearPreview=true){
 const today=todayIsoDate();
 const fields={promoEditId:'',promoTitleInput:'',promoStartDateInput:today,promoEndDateInput:addDaysIso(today,30),promoSortInput:'0'};
 Object.entries(fields).forEach(([id,value])=>{const el=document.getElementById(id);if(el)el.value=value});
 const active=document.getElementById('promoActiveInput');if(active)active.checked=true;
 const file=document.getElementById('promoImageInput');if(file)file.value='';
 const preview=document.getElementById('adminPromoPreview');if(preview)delete preview.dataset.pending;
 if(clearPreview)setPromoImage(preview,document.getElementById('adminPromoEmpty'),'');
 const btn=document.getElementById('savePromoBtn');if(btn)btn.textContent='Simpan Banner';
}
function editPromoBanner(id){
 const row=promoAdminRows.find(x=>String(x.id)===String(id));if(!row)return;
 document.getElementById('promoEditId').value=row.id;document.getElementById('promoTitleInput').value=row.title||'';document.getElementById('promoStartDateInput').value=row.start_date||todayIsoDate();document.getElementById('promoEndDateInput').value=row.end_date||todayIsoDate();document.getElementById('promoSortInput').value=Number(row.sort_order)||0;document.getElementById('promoActiveInput').checked=row.is_active!==false;
 setPromoImage(document.getElementById('adminPromoPreview'),document.getElementById('adminPromoEmpty'),row.image_data||'');document.getElementById('adminPromoPreview').dataset.pending=row.image_data||'';document.getElementById('savePromoBtn').textContent='Simpan Perubahan';document.getElementById('adminPromoCard')?.scrollIntoView({behavior:'smooth',block:'start'})
}
async function savePromoBanner(){
 if(currentUser?.role!=="admin")return toast("Hanya Admin yang dapat mengubah promo");
 const auth=getSbSession();if(!auth?.session_token)return toast("Sesi Admin tidak tersedia. Silakan login ulang");
 const btn=document.getElementById("savePromoBtn"),original=btn?.textContent||"Simpan Banner";if(btn){btn.disabled=true;btn.textContent="Menyimpan..."}
 try{
  const id=document.getElementById('promoEditId')?.value||null,title=(document.getElementById("promoTitleInput")?.value||"").trim()||"Promo Terbaru",start=document.getElementById('promoStartDateInput')?.value,end=document.getElementById('promoEndDateInput')?.value;
  if(!start||!end)throw new Error('Tanggal mulai dan berakhir wajib diisi');if(end<start)throw new Error('Tanggal berakhir tidak boleh sebelum tanggal mulai');
  const preview=document.getElementById('adminPromoPreview');let imageData=preview?.dataset.pending||'';const file=document.getElementById('promoImageInput')?.files?.[0];if(file)imageData=await readPromoFileAsCompressedDataUrl(file);if(!imageData)throw new Error("Pilih gambar promo terlebih dahulu");
  await rpcWithTimeout("app_admin_save_promo_banner",{p_token:auth.session_token,p_id:id,p_title:title,p_image_data:imageData,p_start_date:start,p_end_date:end,p_sort_order:Number(document.getElementById('promoSortInput')?.value||0),p_is_active:document.getElementById('promoActiveInput')?.checked!==false},30000);
  promoBannerCache=[];resetPromoForm();await loadAdminPromoBanners();renderAdminPromoGallery();toast(id?'Banner berhasil diperbarui':'Banner berhasil ditambahkan')
 }catch(e){console.error(e);toast(`Gagal menyimpan promo: ${e.message||e}`)}finally{if(btn){btn.disabled=false;btn.textContent=original.includes('Perubahan')?'Simpan Perubahan':'Simpan Banner'}}
}
async function deletePromoBanner(id){
 if(currentUser?.role!=="admin"||!id)return;if(!confirm("Hapus banner promo ini?"))return;
 const auth=getSbSession();if(!auth?.session_token)return toast('Sesi Admin tidak tersedia');
 try{await rpcWithTimeout("app_admin_delete_promo_banner",{p_token:auth.session_token,p_id:id},20000);promoBannerCache=[];await loadAdminPromoBanners();renderAdminPromoGallery();if(document.getElementById('promoEditId')?.value===String(id))resetPromoForm();toast("Banner berhasil dihapus")}
...
// v1.5.7 Fullscreen promo image viewer
let promoViewerScale=1;
let promoViewerX=0;
let promoViewerY=0;
let promoViewerDragging=false;
let promoViewerStartX=0;
let promoViewerStartY=0;
let promoViewerBaseX=0;
let promoViewerBaseY=0;
let promoViewerPinchDistance=0;
let promoViewerPinchScale=1;

function applyPromoFullscreenTransform(){
 const img=document.getElementById('promoFullscreenImage');
 if(!img)return;
 if(promoViewerScale<=1){promoViewerX=0;promoViewerY=0;}
 img.style.transform=`translate3d(${promoViewerX}px,${promoViewerY}px,0) scale(${promoViewerScale})`;
 const reset=document.getElementById('promoZoomResetBtn');
 if(reset)reset.textContent=`${Math.round(promoViewerScale*100)}%`;
}
function resetPromoFullscreenZoom(){promoViewerScale=1;promoViewerX=0;promoViewerY=0;applyPromoFullscreenTransform()}
function zoomPromoFullscreen(delta,originX=null,originY=null){
 const old=promoViewerScale;
 const next=Math.min(5,Math.max(1,old+delta));
 if(next===old)return;
 const stage=document.getElementById('promoFullscreenStage');
 if(originX!=null&&originY!=null&&stage){
  const rect=stage.getBoundingClientRect();
  const px=originX-(rect.left+rect.width/2);
  const py=originY-(rect.top+rect.height/2);
  const ratio=next/old;
  promoViewerX=px-(px-promoViewerX)*ratio;
  promoViewerY=py-(py-promoViewerY)*ratio;
 }
 promoViewerScale=next;applyPromoFullscreenTransform();
}
function openPromoFullscreen(){
 const row=promoBannerCache?.[promoSlideIndex];
 const src=row?.image_data||document.getElementById('salesPromoImage')?.src;
 if(!src)return;
 const modal=document.getElementById('promoFullscreen'),img=document.getElementById('promoFullscreenImage');
 if(!modal||!img)return;
 stopPromoCarousel();resetPromoFullscreenZoom();img.src=src;img.alt=row?.title||'Promo produk';
 modal.classList.remove('hidden');document.body.classList.add('promo-viewer-open');
 document.getElementById('promoFullscreenClose')?.focus();
}
function closePromoFullscreen(){
 const modal=document.getElementById('promoFullscreen');if(!modal)return;
 modal.classList.add('hidden');document.body.classList.remove('promo-viewer-open');resetPromoFullscreenZoom();
 if(promoBannerCache.length>1)startPromoCarousel();
 document.getElementById('salesPromoImage')?.focus();
}
function promoTouchDistance(touches){if(touches.length<2)return 0;const dx=touches[0].clientX-touches[1].clientX,dy=touches[0].clientY-touches[1].clientY;return Math.hypot(dx,dy)}

document.addEventListener('DOMContentLoaded',()=>{
 const modal=document.getElementById('promoFullscreen');
 const stage=document.getElementById('promoFullscreenStage');
 const img=document.getElementById('promoFullscreenImage');
 if(!modal||!stage||!img)return;
 modal.addEventListener('click',e=>{if(e.target===modal)closePromoFullscreen()});
 stage.addEventListener('dblclick',e=>{e.preventDefault();promoViewerScale>1?resetPromoFullscreenZoom():zoomPromoFullscreen(1,e.clientX,e.clientY)});
 stage.addEventListener('wheel',e=>{e.preventDefault();zoomPromoFullscreen(e.deltaY<0?0.25:-0.25,e.clientX,e.clientY)},{passive:false});
 stage.addEventListener('pointerdown',e=>{if(e.pointerType==='touch')return;if(promoViewerScale<=1)return;promoViewerDragging=true;promoViewerStartX=e.clientX;promoViewerStartY=e.clientY;promoViewerBaseX=promoViewerX;promoViewerBaseY=promoViewerY;stage.setPointerCapture?.(e.pointerId)});
 stage.addEventListener('pointermove',e=>{if(!promoViewerDragging)return;promoViewerX=promoViewerBaseX+(e.clientX-promoViewerStartX);promoViewerY=promoViewerBaseY+(e.clientY-promoViewerStartY);applyPromoFullscreenTransform()});
 const stopDrag=e=>{promoViewerDragging=false;try{stage.releasePointerCapture?.(e.pointerId)}catch(_){}};
 stage.addEventListener('pointerup',stopDrag);stage.addEventListener('pointercancel',stopDrag);
 stage.addEventListener('touchstart',e=>{
  if(e.touches.length===2){promoViewerPinchDistance=promoTouchDistance(e.touches);promoViewerPinchScale=promoViewerScale;}
  else if(e.touches.length===1&&promoViewerScale>1){const t=e.touches[0];promoViewerDragging=true;promoViewerStartX=t.clientX;promoViewerStartY=t.clientY;promoViewerBaseX=promoViewerX;promoViewerBaseY=promoViewerY;}
 },{passive:true});
 stage.addEventListener('touchmove',e=>{
  if(e.touches.length===2&&promoViewerPinchDistance){e.preventDefault();const distance=promoTouchDistance(e.touches);promoViewerScale=Math.min(5,Math.max(1,promoViewerPinchScale*(distance/promoViewerPinchDistance)));applyPromoFullscreenTransform();}
  else if(e.touches.length===1&&promoViewerDragging&&promoViewerScale>1){e.preventDefault();const t=e.touches[0];promoViewerX=promoViewerBaseX+(t.clientX-promoViewerStartX);promoViewerY=promoViewerBaseY+(t.clientY-promoViewerStartY);applyPromoFullscreenTransform();}
 },{passive:false});
 stage.addEventListener('touchend',e=>{if(e.touches.length<2){promoViewerPinchDistance=0;}if(e.touches.length===0)promoViewerDragging=false},{passive:true});
});
window.addEventListener('keydown',e=>{if(e.key==='Escape'&&!document.getElementById('promoFullscreen')?.classList.contains('hidden'))closePromoFullscreen()});

document.addEventListener("DOMContentLoaded",()=>{
 const carousel=document.querySelector("#salesPromoCard .promo-carousel");
 if(!carousel)return;
 let startX=0,startY=0;
 carousel.addEventListener("touchstart",event=>{const t=event.touches?.[0];if(!t)return;startX=t.clientX;startY=t.clientY},{passive:true});
 carousel.addEventListener("touchend",event=>{const t=event.changedTouches?.[0];if(!t)return;const dx=t.clientX-startX,dy=t.clientY-startY;if(Math.abs(dx)>45&&Math.abs(dx)>Math.abs(dy))changePromoSlide(dx<0?1:-1)},{passive:true});
});
...
if("serviceWorker" in navigator&&location.protocol.startsWith("http")){
 window.addEventListener("load",async()=>{
  try{
   const registration=await navigator.serviceWorker.register("./service-worker.js?v=1.5.7",{updateViaCache:"none"});
