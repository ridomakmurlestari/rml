...
async function showPriceList(){
 ...
 if(admin)applyProductAssignmentPanelState();
 document.getElementById('productAdminTitleActions')?.classList.toggle('hidden',!admin);
 const activeArea=getDailyArea();
 const activeCustomerNo=activeProductCustomerNo();const activeCustomer=customers().find(c=>String(c.no)===String(activeCustomerNo));
 document.getElementById('priceListSubtitle').textContent=admin?'Kelola master barang, kategori, harga, dan pembagian per Sales, Area, serta Toko.':activeCustomer?`Daftar harga khusus ${activeCustomer.name}.`:activeArea?`Daftar harga untuk area ${activeArea}.`:'Pilih area tugas terlebih dahulu untuk melihat daftar harga.';
...
function productsVisibleForCurrentUser(){
 ...
 if(!area)return [];
 const customerNo=activeProductCustomerNo();
 const ids=new Set(assignedProductIds(currentUser?.email,area,customerNo).map(String));
...
function findProductImportColumns(rows){
 ...
 for(let r=0;r<Math.min(rows.length,30);r++){
  ...
  const barcode=find(aliases.barcode),name=find(aliases.name),price=find(aliases.price);
  if(price>=0&&(barcode>=0||name>=0))return {headerRow:r,barcode,name,price};
 }
 return null;
}
 ...
 try{
  ...
  for(const sheetName of workbook.SheetNames){
   ...
   for(let i=columns.headerRow+1;i<rows.length;i++){
    ...
    imported.push({barcode,name,price,row:i+1,sheet:sheetName});
   }
  }
  ...
  for(const item of imported){
   ...
   if(!Number.isFinite(item.price)||item.price<0){invalid++;continue}
   if(Number(products[index].price)===item.price){unchanged++;continue}
   products[index]={...products[index],price:item.price};
   updated++;
  }
...
function renderProductAssignments(){
 ...
 if(!email){document.getElementById('productAssignmentList').innerHTML='<div class="empty">Belum ada akun Sales.</div>';const c=document.getElementById('productAssignmentCollapseSummary');if(c)c.textContent='Belum ada akun Sales.';return}
 if(!area){document.getElementById('productAssignmentList').innerHTML='<div class="empty">Sales ini belum memiliki area tugas.</div>';const c=document.getElementById('productAssignmentCollapseSummary');if(c)c.textContent='Sales ini belum memiliki area tugas.';return}
