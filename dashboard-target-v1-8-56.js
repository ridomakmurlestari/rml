/* RML Sales Visit v1.8.56 - Target Reward Bertingkat - Supervisor Owner Fix */
(function(){
...
function rewardHtml(x){const tiers=targetTiers(x);if(!tiers.length)return '';const hit=achievedTier(x),next=tiers.find(t=>Number(t.target||0)>Number(x.achieved||0));if(hit){return `<div class="target-reward-box"><div><small>Reward tercapai</small><strong>${money(tierRewardAmount(hit))}</strong><span>Target ${money(hit.target)}</span></div>${next?`<div><small>Reward berikutnya</small><strong>${money(tierRewardAmount(next))}</strong><span>Target ${money(next.target)}</span></div>`:''}</div>`}const first=tiers[0];return `<div class="target-reward-box"><div><small>Reward berikutnya</small><strong>${money(tierRewardAmount(first))}</strong><span>Target ${money(first.target)}</span></div></div>`}
function rewardSummaryText(x){const tiers=targetTiers(x);if(!tiers.length)return '';const hit=achievedTier(x);const next=tiers.find(t=>Number(t.target||0)>Number(x.achieved||0));if(hit)return `Reward: ${money(tierRewardAmount(hit))}${next?` • Berikutnya ${money(tierRewardAmount(next))}`:''}`;return `Reward ${money(tierRewardAmount(tiers[0]))}`}
...
function resetTargetForm(){document.getElementById('targetEditId').value='';document.getElementById('targetTypeInput').value='sales';document.getElementById('targetMonthInput').value=monthNow();document.getElementById('targetDurationInput').value='1';document.getElementById('targetAmountInput').value='';document.getElementById('targetAchievedInput').value='';document.getElementById('targetRewardTypeInput').value='none';document.getElementById('targetRewardValueInput').value='';renderTargetTiersForm([{target:'',rewardType:'nominal',rewardValue:''}]);document.getElementById('targetDescriptionInput').value='';document.getElementById('targetUpdatedDateInput').value=new Date().toISOString().slice(0,10);fillTargetManagerFields()}
