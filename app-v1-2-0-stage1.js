...
function createOrderRowHtml(item={barang:"",jumlah:""}){
 return `<tr class="order-entry-row">
   <td><input class="order-item-name" type="text" value="${esc(item.barang||"")}" placeholder="Nama barang" autocomplete="off"></td>
   <td><input class="order-item-qty" type="text" value="${esc(item.jumlah||"")}" placeholder="Contoh: 2 box" autocomplete="off"></td>
   <td><button class="order-remove-btn" type="button" aria-label="Hapus barang" onclick="removeOrderRow(this)">×</button></td>
 </tr>`;
}
function addOrderRow(containerId,item={barang:"",jumlah:""}){
 const body=document.getElementById(containerId);if(!body)return;
 body.insertAdjacentHTML("beforeend",createOrderRowHtml(item));
 const row=body.lastElementChild;row?.querySelector(".order-item-name")?.focus();
}
function removeOrderRow(button){
 const body=button?.closest("tbody");button?.closest("tr")?.remove();
 if(body&&!body.children.length)addOrderRow(body.id);
}
function resetOrderRows(containerId,items=[]){
 const body=document.getElementById(containerId);if(!body)return;
 body.innerHTML="";(items.length?items:[{}]).forEach(item=>addOrderRow(containerId,item));
}
function readOrderRows(containerId){
 const body=document.getElementById(containerId);if(!body)return [];
 return [...body.querySelectorAll("tr")].map(row=>({
   barang:row.querySelector(".order-item-name")?.value.trim()||"",
   jumlah:row.querySelector(".order-item-qty")?.value.trim()||""
 })).filter(item=>item.barang||item.jumlah);
}
function validateOrderRows(containerId){
 const rows=readOrderRows(containerId);
 if(!rows.length){toast("Tambahkan minimal satu barang");return null}
 const invalid=rows.find(item=>!item.barang||!item.jumlah);
 if(invalid){toast("Nama barang dan jumlah wajib diisi");return null}
 return rows;
}
function orderItemsToNote(items){return items.map(item=>`${item.barang} — ${item.jumlah}`).join("\n")}
function addVisitOrderRow(){addOrderRow("visitOrderRows")}
function addOutsideOrderRow(){addOrderRow("outsideOrderRows")}
