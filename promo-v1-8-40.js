/* RML Sales Visit v1.8.40 - Promo Bulan Ini: per Penanggung Jawab + kategori */
(function(){
  ...
  function openDetail(email){const categories=categoriesForOwner(email);if(!flatItems(categories).length)return;const name=userName(email);const salesEl=document.getElementById('promoDetailSales');if(salesEl)salesEl.textContent=name;const monthEl=document.getElementById('promoDetailMonth');if(monthEl)monthEl.textContent=monthLabel(monthKey());const list=document.getElementById('promoDetailList');if(list)list.innerHTML=categories.filter(c=>c.items.length).map(c=>`<div class="promo-detail-category"><h4>${escP(c.name)}</h4>${c.items.map((x,i)=>`<div class="promo-detail-item"><span>${i+1}</span><strong>${escP(x)}</strong></div>`).join('')}</div>`).join('');document.getElementById('promoDetailModal')?.classList.remove('hidden')}
  ...
  function readCategories(){return [...document.querySelectorAll('#promoCategories .promo-category-editor')].map((el,i)=>({name:String(el.querySelector('.promo-category-name')?.value||'').trim()||`Kategori ${i+1}`,items:String(el.querySelector('.promo-category-items')?.value||'').split(/\r?\n/).map(x=>x.trim()).filter(Boolean)})).filter(c=>c.items.length)}
  function addCategory(){const cats=readCategories();cats.push({name:'',items:[]});renderCategories(cats);document.querySelectorAll('#promoCategories .promo-category-name').at(-1)?.focus()}
