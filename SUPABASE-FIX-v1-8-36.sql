-- RML Sales Visit v1.8.36 - USER/TARGET/PROMO SERVER FIX
...
begin
 select lower(s.role) into v_role from public._app_session_user(p_token) s;
 if v_role not in ('admin','supervisor','sales') then raise exception 'Sesi login tidak valid'; end if;
 if v_role='admin' then
   ...
   return query select u.account_key,u.login_name,u.display_name,u.phone,u.role,u.active,u.must_change_password,u.can_switch_area_freely from public.app_users u where u.role='sales' or lower(u.account_key)=lower((select s.account_key from public._app_session_user(p_token) s limit 1)) order by u.role,u.display_name;
 else
   return query select u.account_key,u.login_name,u.display_name,u.phone,u.role,u.active,u.must_change_password,u.can_switch_area_freely from public.app_users u where u.id=(select s.user_id from public._app_session_user(p_token) s limit 1);
 end if;
end $$;
