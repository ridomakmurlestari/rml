...
function loadPreview(){
 ...
 document.getElementById("reportRows").innerHTML=rows.length?rows.map(r=>{
   const changes=(r.editHistory||[]).map(e=>`<div class="edit-line">${esc(e.editedAt)} - ${esc(e.editedBy)}: ${esc(e.oldNote)} → ${esc(e.newNote)}</div>`).join("");
   return `<tr><td>${esc(r.no)}</td><td><strong>${esc(r.code)}</strong><br>${esc(r.outlet)}<br><small>${esc(r.area)}</small></td><td>${esc(r.sales)}</td><td>${esc(r.status)}</td><td>Masuk: ${esc(r.checkIn)}<br>Keluar: ${esc(r.checkOut)}<br>Durasi: ${esc(r.duration)}</td><td><strong>${esc(r.detailLabel)}</strong><br>${esc(r.note)}${changes?`<br><small>Riwayat perubahan:</small>${changes}`:""}</td></tr>`;
