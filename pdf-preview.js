...
function orderHtml(value,deliveryItems=[]){const lines=cleanOrderLines(value);return lines.length?`<div class="pdf-order-list">${lines.map((line,index)=>{const delivered=Boolean(deliveryItems?.[index]?.delivered);return `<div class="pdf-order-row"><span>• ${esc(line)}</span><strong>${delivered?"✓ Dikirim":"Belum dikirim"}</strong></div>`;}).join("")}</div>`:"-";}
function orderText(value,deliveryItems=[]){const lines=cleanOrderLines(value);return lines.length?lines.map((line,index)=>`• ${line} — ${deliveryItems?.[index]?.delivered?"Dikirim":"Belum dikirim"}`).join("\n"):"-";}
...
async function savePreviewAsPdf(){
 ...
 try{
   ...
   const body=(previewData.rows||[]).map(r=>[String(r.no),`${r.code}\n${r.outlet}\n${r.area}`,r.sales,r.status,`${r.checkIn}\nKeluar: ${r.checkOut}\nDurasi: ${r.duration}`,`${r.detailLabel}${r.paymentStatus&&r.paymentStatus!=="-"?` | Pembayaran: ${r.paymentStatus}`:""}\n${r.status==="Ada Order"?orderText(r.note,r.deliveryItems):r.note}`]);
