...
function productAssignmentKey(email,area){return `${String(email||"").trim()}@@${String(area||"").trim()}`}
function assignedProductIds(email,area){
 const map=productAssignments();
 const specific=map[productAssignmentKey(email,area)];
...
async function showPriceList(){
 ...
 const activeArea=getDailyArea();
 document.getElementById('priceListSubtitle').textContent=admin?'Kelola master barang, kategori, harga, dan pembagian per Sales serta Area.':activeArea?`Daftar harga untuk area ${activeArea}.`:'Pilih area tugas terlebih dahulu untuk melihat daftar harga.';
...
function productsVisibleForCurrentUser(){
 ...
 if(!area)return [];
 const ids=new Set(assignedProductIds(currentUser?.email,area).map(String));
 return all.filter(x=>x.active!==false&&ids.has(String(x.id)));
}
...
async function saveProductFromModal(){
 ...
 const payload={code:document.getElementById('productModalCodeInput').value.trim(),name,category,price:Math.round(price),active:document.getElementById('productModalActiveInput').checked};
 let message='';
 if(id){
  const i=rows.findIndex(x=>String(x.id)===String(id));if(i<0){if(btn){btn.disabled=false;btn.textContent='Simpan Perubahan'}return toast('Barang tidak ditemukan')}
  rows[i]={...rows[i],...payload};
  message='Perubahan produk berhasil disimpan';
 }else{
  rows.push({id:makeProductId(),...payload});
 ...
 toast(ok?message:`${message} di perangkat dan akan disinkronkan saat online`);
...
function renderProductAssignments(){
 ...
 if(!area){document.getElementById('productAssignmentList').innerHTML='<div class="empty">Sales ini belum memiliki area tugas.</div>';return}
 ...
 const selected=new Set(assignedProductIds(email,area).map(String));
 ...
 const summary=document.getElementById('productAssignmentSummary');if(summary)summary.textContent=`${email.split('@')[0]} · ${area} · ${rows.length} barang tampil · ${rows.filter(x=>selected.has(String(x.id))).length} dipilih`;
 document.getElementById('productAssignmentList').innerHTML=rows.map(x=>`<label class="product-assignment-item"><input type="checkbox" ${selected.has(String(x.id))?'checked':''} onchange="toggleProductAssignment('${esc(email)}','${esc(area)}','${esc(x.id)}',this.checked)"><span><strong>${esc(x.name)}</strong><small>${esc(x.category)} · ${esc(x.code||'Tanpa Barcode')} · ${formatProductPrice(x.price)}</small></span></label>`).join('')||'<div class="empty">Tidak ada barang pada filter ini.</div>';
}

async function toggleProductAssignment(email,area,id,checked){
 const map=productAssignments(),key=productAssignmentKey(email,area),set=new Set(assignedProductIds(email,area).map(String));
...
async function setAllVisibleProductAssignments(checked){
 const email=document.getElementById('productSalesFilter')?.value,area=document.getElementById('productAreaFilter')?.value;if(!email||!area)return;
 ...
 const map=productAssignments(),key=productAssignmentKey(email,area),set=new Set(assignedProductIds(email,area).map(String));visible.forEach(id=>checked?set.add(id):set.delete(id));map[key]=[...set];saveProductAssignmentsLocal(map);renderProductAssignments();
...
async function copyProductAssignmentsFromArea(){
 const email=document.getElementById('productSalesFilter')?.value,targetArea=document.getElementById('productAreaFilter')?.value,sourceArea=document.getElementById('productCopyAreaFilter')?.value;
 ...
 const map=productAssignments();map[productAssignmentKey(email,targetArea)]=[...assignedProductIds(email,sourceArea).map(String)];saveProductAssignmentsLocal(map);renderProductAssignments();
 const ok=navigator.onLine?await syncProductsToSupabase({silent:true}):false;toast(ok?'Pembagian area berhasil disalin':'Pembagian tersimpan lokal dan akan disinkronkan saat online');if(!ok)scheduleProductSync();
}
