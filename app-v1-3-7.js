...
function renderProductAssignments(){
 ...
 const summary=document.getElementById('productAssignmentSummary');if(summary)summary.textContent=`${email.split('@')[0]} · ${area} · ${scope} · ${rows.length} barang tampil · ${rows.filter(x=>selected.has(String(x.id))).length} dipilih`;
