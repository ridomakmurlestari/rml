...
const APP_VERSION="1.2.0-stage1";
...
function createOrderRowHtml(item={barang:"",jumlah:""}){
 return `<tr class="order-entry-row">
   <td class="order-item-index"></td>
   <td class="order-item-name-cell">${esc(item.barang||"")}</td>
   <td class="order-item-qty-cell">${esc(item.jumlah||"")}</td>
   <td><button class="order-remove-btn" type="button" aria-label="Hapus barang" onclick="removeOrderRow(this)">×</button></td>
 </tr>`;
}
function refreshOrderRowNumbers(containerId){
 const body=document.getElementById(containerId);if(!body)return;
 [...body.querySelectorAll("tr")].forEach((row,index)=>{const cell=row.querySelector(".order-item-index");if(cell)cell.textContent=String(index+1)});
 const wrap=body.closest(".order-result-wrap");if(wrap)wrap.classList.toggle("hidden",!body.children.length);
}
function addOrderItem(containerId,item){
 const body=document.getElementById(containerId);if(!body)return;
 body.insertAdjacentHTML("beforeend",createOrderRowHtml(item));refreshOrderRowNumbers(containerId);
}
function removeOrderRow(button){
 const body=button?.closest("tbody");button?.closest("tr")?.remove();if(body)refreshOrderRowNumbers(body.id);
}
function getOrderInputIds(containerId){
 return containerId==="outsideOrderRows"?{name:"outsideOrderName",qty:"outsideOrderQty"}:{name:"visitOrderName",qty:"visitOrderQty"};
}
function addOrderFromInputs(containerId){
 const ids=getOrderInputIds(containerId),nameInput=document.getElementById(ids.name),qtyInput=document.getElementById(ids.qty);
 if(!nameInput||!qtyInput)return false;
 const barang=nameInput.value.trim(),jumlah=qtyInput.value.trim();
 if(!barang&&!jumlah)return false;
 if(!barang){toast("Nama barang wajib diisi");nameInput.focus();return false}
 if(!jumlah){toast("Jumlah wajib diisi");qtyInput.focus();return false}
 addOrderItem(containerId,{barang,jumlah});nameInput.value="";qtyInput.value="";nameInput.focus();return true;
}
function handleOrderInputKey(event,containerId){
 if(event.key!=="Enter")return;
 event.preventDefault();addOrderFromInputs(containerId);
}
function resetOrderRows(containerId,items=[]){
 const body=document.getElementById(containerId);if(!body)return;
 body.innerHTML="";(items||[]).forEach(item=>addOrderItem(containerId,item));refreshOrderRowNumbers(containerId);
 const ids=getOrderInputIds(containerId);const nameInput=document.getElementById(ids.name),qtyInput=document.getElementById(ids.qty);
 if(nameInput)nameInput.value="";if(qtyInput)qtyInput.value="";
}
function readOrderRows(containerId){
 const body=document.getElementById(containerId);if(!body)return [];
 return [...body.querySelectorAll("tr")].map(row=>({
   barang:row.querySelector(".order-item-name-cell")?.textContent.trim()||"",
   jumlah:row.querySelector(".order-item-qty-cell")?.textContent.trim()||""
 })).filter(item=>item.barang||item.jumlah);
}
function validateOrderRows(containerId){
 const ids=getOrderInputIds(containerId),nameInput=document.getElementById(ids.name),qtyInput=document.getElementById(ids.qty);
 if((nameInput?.value||"").trim()||(qtyInput?.value||"").trim()){
   if(!addOrderFromInputs(containerId))return null;
 }
 const rows=readOrderRows(containerId);
 if(!rows.length){toast("Masukkan minimal satu barang, lalu tekan Enter pada kolom Jumlah");nameInput?.focus();return null}
 return rows;
}
function orderItemsToNote(items){return items.map(item=>`${item.barang} — ${item.jumlah}`).join("\n")}
function selectedPaymentStatus(){return document.querySelector('input[name="visitPaymentStatus"]:checked')?.value||""}
function resetPaymentStatus(){document.querySelectorAll('input[name="visitPaymentStatus"]').forEach(input=>input.checked=false)}
...
function openOutsideAreaOrder(){
 ...
 fillOutsideOrderCustomers();resetOrderRows("outsideOrderRows");
...
async function saveOutsideAreaOrder(){
 const c=customers().find(x=>String(x.no)===String(outsideOrderCustomer.value));if(!c)return toast("Pilih pelanggan");
 const orderItems=validateOrderRows("outsideOrderRows");if(!orderItems)return;
 const note=orderItemsToNote(orderItems),now=new Date().toISOString();
 await saveVisitOffline({id:`OUT-${Date.now()}-${c.no}`,customerNo:c.no,code:c.code||"",name:c.name||"",area:c.area,salesEmail:currentUser.email,salesName:currentUser.name,checkInAt:now,checkOutAt:now,durationMinutes:0,status:"Ada Order",note,orderItems,paymentStatus:"",visitState:"completed",createdAt:now,syncStatus:"pending",completionType:"outside-area-order",visitType:"Order Luar Area"});
 await refreshVisitCache();toast("Order luar area berhasil disimpan");showCustomers();
}
...
function showCheckOutPanel(active){
 ...
 resetOrderRows("visitOrderRows");
 resetPaymentStatus();
 updateVisitNoteField();
}
function updateVisitNoteField(){
 const status=document.getElementById("visitStatus").value;
 const noteContainer=document.getElementById("visitNoteContainer");
 const orderContainer=document.getElementById("visitOrderContainer");
 const label=document.getElementById("visitNoteLabel");
 const note=document.getElementById("visitNote");

 orderContainer.classList.toggle("hidden",status!=="Ada Order");
 if(status==="Ada Order"){
   noteContainer.classList.add("hidden");
   note.value="";
   if(!document.getElementById("visitOrderRows")?.children.length)resetOrderRows("visitOrderRows");
   return;
 }
 if(!status){
   noteContainer.classList.add("hidden");
   label.textContent="Catatan *";note.placeholder="Tuliskan catatan...";note.value="";enableOrderBulletInput(note,false);return;
 }
 noteContainer.classList.remove("hidden");
 if(status==="Tidak Order"){
   label.textContent="Alasan Tidak Order *";note.placeholder="Tuliskan alasan tidak order...";
 }else{
   label.textContent="Catatan *";note.placeholder="Tuliskan catatan...";
 }
...
async function confirmCheckOut(){
 ...
 let note="",orderItems=[];
 if(status==="Ada Order"){
   orderItems=validateOrderRows("visitOrderRows");if(!orderItems)return;
   note=orderItemsToNote(orderItems);
 }else{
   const noteInput=document.getElementById("visitNote");note=noteInput.value.trim();
   if(!note){toast(status==="Tidak Order"?"Alasan tidak order wajib diisi.":"Catatan wajib diisi.");noteInput.focus();return}
 }

 const checkOut=new Date();
 const completed={...active,status,note,orderItems,paymentStatus,checkOutAt:checkOut.toISOString(),durationMinutes:calculateDurationMinutes(active.checkInAt,checkOut.toISOString()),visitState:"completed",createdAt:checkOut.toISOString(),syncStatus:"pending"};
 try{await saveVisitOffline(completed);clearActiveVisit()}catch(e){console.error(e);return toast("Gagal menyimpan data kunjungan di HP")}
 toast(`Check Out berhasil. Durasi ${completed.durationMinutes} menit`);setTimeout(showCustomers,500);
}
...
["visitNote","editVisitNoteInput"].forEach(id=>{
