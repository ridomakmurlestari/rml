...
const REMOTE_VISIT_INITIAL_LIMIT=5000;
...
const REMOTE_SYNC_META_KEY="rml_remote_sync_v1";
const VISIT_FULL_SYNC_VERSION="v2";
function getRemoteSyncMeta(){try{const v=JSON.parse(localStorage.getItem(REMOTE_SYNC_META_KEY)||"{}");return v&&typeof v==="object"?v:{}}catch(_){return {}}}
function setRemoteSyncMeta(patch){const next={...getRemoteSyncMeta(),...(patch||{})};localStorage.setItem(REMOTE_SYNC_META_KEY,JSON.stringify(next));return next}
function clearRemoteSyncMeta(){localStorage.removeItem(REMOTE_SYNC_META_KEY)}
function maxVisitUpdatedAt(rows){let max=null;for(const r of rows||[]){const v=r?.updatedAt||r?.updated_at;if(!v)continue;if(!max||new Date(v)>new Date(max))max=v}return max}
...
async function pullRemoteVisits({reconcile=false}={}){
 ...
 const patch={visits_full_sync_version:VISIT_FULL_SYNC_VERSION};
 if(newest)patch.visits_updated_at=newest;
 setRemoteSyncMeta(patch);
 return rows||[];
}
...
function visitRetentionCutoff(){
 const d=new Date();
 d.setHours(0,0,0,0);
 d.setDate(d.getDate()-VISIT_RETENTION_DAYS);
 return d;
}
async function cleanupLocalVisitsOlderThan30Days(){
 const cutoff=visitRetentionCutoff();
 const rows=await idbGetAll(STORE_VISITS);
 let removed=0;
 for(const visit of rows){
  const date=visitDateValue(visit);
  if(date&&date<cutoff){await idbDelete(STORE_VISITS,visit.id);removed++;}
 }
 if(removed)await refreshVisitCache();
 return removed;
}
async function cleanupVisitsOlderThan30Days({remote=true,silent=true}={}){
 let localRemoved=0,remoteRemoved=0;
 try{localRemoved=await cleanupLocalVisitsOlderThan30Days();}catch(e){console.warn('Pembersihan riwayat lokal gagal',e);}
 if(remote&&navigator.onLine){
  const session=getSbSession();
  if(session?.session_token){
   try{
    const result=await rpc('app_cleanup_visits_older_than_30_days',{p_token:session.session_token});
    remoteRemoved=Number(Array.isArray(result)?result[0]:result)||0;
   }catch(e){
    console.warn('Pembersihan riwayat Supabase gagal',e);
    if(!silent)toast(`Gagal membersihkan riwayat lama: ${e.message||'Periksa SQL Supabase'}`);
   }
  }
 }
...
async function syncSettingsToSupabase(){
 const session=getSbSession();
 if(!(currentUser?.role==='admin'||isSupervisorUser())||!navigator.onLine||!session?.session_token)return;
 const users=USERS.map(u=>({account_key:u.email,display_name:u.name,login_name:(u.loginName||u.name).trim().toLowerCase(),phone:u.phone,role:u.role,active:u.active!==false,can_switch_area_freely:u.canSwitchAreaFreely===true}));
 const map=getAreaAssignments();
 const assignments=[];
 for(const u of USERS.filter(x=>x.role==='sales'||x.role==='supervisor'))for(const area of (map[u.email]||[]))assignments.push({sales_email:u.email,area});
 // Simpan ke sistem pengaturan lama, tetapi jangan biarkan kegagalan di sana
 // menghentikan sinkronisasi tabel assignment area yang khusus.
 try{await rpc('app_admin_save_settings',{p_token:session.session_token,p_users:users,p_assignments:assignments})}
 catch(e){console.warn('Pengaturan utama gagal disimpan:',e.message||e)}
 try{await rpc('app_admin_set_area_assignments',{p_token:session.session_token,p_assignments:assignments})}
 catch(e){console.warn('Remote area sync RPC belum tersedia / gagal:',e.message||e);throw e}
}
const AREA_ASSIGNMENT_REMOTE_READY_KEY='rml_area_assignments_remote_ready_v1';
function markRemoteAreaAssignmentReady(email){if(!email)return;try{const m=JSON.parse(localStorage.getItem(AREA_ASSIGNMENT_REMOTE_READY_KEY)||'{}');m[email]=true;localStorage.setItem(AREA_ASSIGNMENT_REMOTE_READY_KEY,JSON.stringify(m))}catch(_){} }
function isRemoteAreaAssignmentReady(email){try{const m=JSON.parse(localStorage.getItem(AREA_ASSIGNMENT_REMOTE_READY_KEY)||'{}');return !!m[email]}catch(_){return false}}
async function pullAreaAssignmentsFromSupabase({silent=true}={}){
 const session=getSbSession(),email=currentUser?.email;
 if(!email||!navigator.onLine||!session?.session_token||!(currentUser?.role==='sales'||currentUser?.role==='supervisor'))return false;
 try{
  const data=await rpc('app_get_my_area_assignments',{p_token:session.session_token});
  const rows=Array.isArray(data)?data:[];
  const areas=[...new Set(rows.map(r=>String(r?.area||r).trim()).filter(Boolean))];
  const map=getAreaAssignments();
  // Supervisor lama mungkin belum pernah tersimpan di tabel remote. Jangan
  // mengosongkan assignment lokal pada pull pertama; setelah admin menyimpan,
  // marker remote akan aktif dan daftar kosong pun dianggap valid.
  if(currentUser?.role==='supervisor'&&areas.length===0&&!isRemoteAreaAssignmentReady(email)){
    if(!Array.isArray(map[email]))map[email]=allAreas();
  }else{
    map[email]=areas;
  }
  localStorage.setItem(AREA_ASSIGNMENT_KEY,JSON.stringify(map));markRemoteAreaAssignmentReady(email);
  const active=getDailyArea(); if(active&&!areas.includes(active))localStorage.removeItem(dailyAreaStorageKey());
  return true;
 }catch(e){if(!silent)toast(`Sinkronisasi penugasan area gagal: ${e.message||'Periksa koneksi'}`);return false}
}
...
async function pullCustomersFromSupabase({silent=true}={}){
 ...
 try{
  const meta=getRemoteSyncMeta();
  const data=await rpc('app_pull_customers_delta',{p_token:session.session_token,p_since:meta.customers_updated_at||null});
  ...
  if(Array.isArray(remote)&&remote.length){
   ...
   if(currentUser?.role==='admin'){
    ...
    if(migrated.changed){
     ...
     // The admin save updates the server timestamp; force one later delta check.
     const m=getRemoteSyncMeta();delete m.customers_updated_at;localStorage.setItem(REMOTE_SYNC_META_KEY,JSON.stringify(m));
    }
   }
   return true;
  }
...
async function pullProductsFromSupabase({silent=true}={}){
 ...
 try{
  const meta=getRemoteSyncMeta();
  const data=await rpc('app_pull_products_delta',{p_token:session.session_token,p_since:meta.products_updated_at||null});
  const row=Array.isArray(data)?data[0]:data;
  if(row?.changed===false){
   if(row.updated_at)setRemoteSyncMeta({products_updated_at:row.updated_at});
   return true;
  }
...
function fillProductCategoryInput(selected=''){
 const el=document.getElementById('productModalCategoryInput');
 if(!el)return;
 const cats=productCategories();
 const current=String(selected||'').trim();
 const options=[`<option value="">Pilih kategori...</option>`].concat(cats.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`));
 if(current && !cats.includes(current))options.push(`<option value="${esc(current)}">${esc(current)}</option>`);
...
function addNewProductCategory(){
 ...
 const value=prompt('Masukkan nama kategori baru:','');
 if(value===null)return;
 const category=String(value).trim();
 if(!category)return toast('Nama kategori belum diisi');
 const existing=productCategories().find(x=>String(x).trim().toLowerCase()===category.toLowerCase());
 if(existing){
   fillProductCategoryInput(existing);
   return toast(`Kategori "${existing}" sudah ada dan dipilih`);
 }
 fillProductCategoryInput(category);
 toast(`Kategori baru "${category}" dipilih. Klik Simpan Barang untuk menambahkannya.`);
}
...
function fillProductFilters(){
 ...
 fillProductCategoryInput(document.getElementById('productModalCategoryInput')?.value||'');
...
async function showPriceList(){
 ...
 document.getElementById('productBackToSettingsBtn')?.classList.toggle('hidden',!admin);
 const title=document.getElementById('priceListTitle');if(title)title.textContent=admin?'Master Produk':'Daftar Harga';
 document.getElementById('priceListSubtitle').textContent=admin
  ?'Kelola master barang, kategori, harga, diskon, status produk, dan import data.'
...
function editProduct(id){
 ...
 fillProductCategoryInput(x.category||'');
...
function loadUsers(){
 try{
  ...
  if(Array.isArray(saved)){
   const mergedDefaults=DEFAULT_USERS.map(def=>{const old=saved.find(x=>x.email===def.email)||{};let merged={...def,...old};merged=normalizeKnownUserRole(merged);if(typeof merged.canSwitchAreaFreely!=="boolean")merged.canSwitchAreaFreely=def.canSwitchAreaFreely===true;return merged});
   const extras=saved.filter(x=>x&&x.email&&!DEFAULT_USERS.some(def=>def.email===x.email)).map(x=>({...x,role:x.role||"sales",active:x.active!==false,canSwitchAreaFreely:x.canSwitchAreaFreely===true ,mustChangePassword:x.mustChangePassword!==false}));
...
let USERS=loadUsers();
function isSupervisorUser(user=currentUser){
...
function defaultAreaAssignments(){const r={};const areas=allAreas();USERS.filter(u=>u.role==="sales").forEach(u=>r[u.email]=[...new Set(customers().filter(c=>!c.isHidden&&(c.assignedSalesEmail===u.email||c.assignedSalesEmail==="__ALL__")).map(c=>c.area).filter(Boolean))]);USERS.filter(u=>u.role==="supervisor").forEach(u=>r[u.email]=[...areas]);return r;}
function getAreaAssignments(){
  let map=null;
  try{
    const raw=localStorage.getItem(AREA_ASSIGNMENT_KEY);
    map=raw?JSON.parse(raw):null;
  }catch(e){map=null}
  if(!map||typeof map!=="object")return defaultAreaAssignments();
  return map;
}
function ensureSalesAreaAssignment(email){
  if(!email)return [];
  const map=getAreaAssignments();
  const current=Array.isArray(map[email])?map[email]:[];
  if(current.length)return current;
  const defaults=defaultAreaAssignments();
  const fallback=Array.isArray(defaults[email])?defaults[email]:[];
  if(fallback.length){
    map[email]=[...new Set(fallback)];
    localStorage.setItem(AREA_ASSIGNMENT_KEY,JSON.stringify(map));
    return map[email];
  }
  return [];
}
function assignedAreasForSales(email){
  const map=getAreaAssignments();
  const current=Array.isArray(map[email])?map[email]:[];
  if(current.length)return current;
  return ensureSalesAreaAssignment(email);
}
function assignedAreasForUser(email){
  const user=USERS.find(u=>u.email===email)||currentUser;
  if(user?.role==="supervisor"){
    const map=getAreaAssignments();
    if(!Array.isArray(map[email])){
      map[email]=allAreas();
      localStorage.setItem(AREA_ASSIGNMENT_KEY,JSON.stringify(map));
    }
    return Array.isArray(map[email])?map[email]:[];
  }
  return assignedAreasForSales(email);
}
function isAreaAssigned(email,area){return assignedAreasForUser(email).includes(area)}
...
function applyAreaAssignmentPanelState(){
 const body=document.getElementById("areaAssignmentBody"),header=document.getElementById("areaAssignmentHeader"),text=document.getElementById("areaAssignmentToggleText"),chevron=document.getElementById("areaAssignmentChevron");
 if(!body||!header)return;const collapsed=isAreaAssignmentCollapsed();body.classList.toggle("collapsed",collapsed);header.classList.toggle("collapsed",collapsed);header.setAttribute("aria-expanded",String(!collapsed));if(text)text.textContent=collapsed?"Buka":"Tutup";if(chevron)chevron.textContent=collapsed?"›":"⌄";
}
function toggleAreaAssignmentPanel(){localStorage.setItem(AREA_ASSIGNMENT_COLLAPSE_KEY,isAreaAssignmentCollapsed()?"0":"1");applyAreaAssignmentPanelState()}
function toggleTargetSettingsPanel(){const body=document.getElementById("targetSettingsBody");if(!body)return;const collapsed=body.classList.toggle("collapsed");document.getElementById("targetSettingsHeader")?.setAttribute("aria-expanded",collapsed?"false":"true");const text=document.getElementById("targetSettingsToggleText");if(text)text.textContent=collapsed?"Buka":"Tutup";const chev=document.getElementById("targetSettingsChevron");if(chev)chev.textContent=collapsed?"›":"⌄";localStorage.setItem("rml_target_settings_collapsed",collapsed?"1":"0");}
function applyTargetSettingsPanelState(){const body=document.getElementById("targetSettingsBody");if(!body)return;const collapsed=localStorage.getItem("rml_target_settings_collapsed")==="1";body.classList.toggle("collapsed",collapsed);document.getElementById("targetSettingsHeader")?.setAttribute("aria-expanded",collapsed?"false":"true");const text=document.getElementById("targetSettingsToggleText");if(text)text.textContent=collapsed?"Buka":"Tutup";const chev=document.getElementById("targetSettingsChevron");if(chev)chev.textContent=collapsed?"›":"⌄";}
function showAreaAssignments(){if(!(currentUser?.role==="admin"||isSupervisorUser()))return;hide();document.getElementById("assignmentView").classList.remove("hidden");renderAreaAssignments();renderUserManagement();applyAreaAssignmentPanelState();}
function renderAreaAssignments(){
 const areas=allAreas(),sales=USERS.filter(u=>u.role==='sales'),supervisors=USERS.filter(u=>u.role==='supervisor'),map=getAreaAssignments(),host=document.getElementById('areaAssignmentMatrix');if(!host)return;
 const supervisorCols=supervisors.map(u=>`<th><div>${esc(u.name)}</div><small class="assignment-role-note">Supervisor</small></th>`).join('');
 host.innerHTML=`<table class="assignment-matrix"><thead><tr><th>Area</th>${sales.map(u=>`<th>${esc(u.name)}</th>`).join("")}${supervisorCols}</tr></thead><tbody>${areas.map(a=>{const encoded=encodeURIComponent(a);return `<tr><td><div class="area-name-cell"><strong>${esc(a)}</strong><button class="area-edit-btn" type="button" onclick="renameAreaName('${encoded}')" aria-label="Edit nama area ${esc(a)}">Edit</button></div></td>${sales.map(u=>`<td><input type="checkbox" ${((map[u.email]||[]).includes(a))?"checked":""} onchange="toggleAreaAssignment('${u.email}','${encoded}',this.checked)"></td>`).join("")}${supervisors.map(u=>`<td><input type="checkbox" ${((map[u.email]||allAreas()).includes(a))?"checked":""} onchange="toggleAreaAssignment('${u.email}','${encoded}',this.checked)"></td>`).join("")}</tr>`}).join("")}</tbody></table>`;
 const summary=document.getElementById("areaAssignmentCollapseSummary");if(summary){const totalChecked=sales.reduce((n,u)=>n+(map[u.email]||[]).length,0);summary.textContent=`${areas.length} area · ${sales.length} sales${supervisors.length?` · ${supervisors.length} supervisor`:''} · ${totalChecked} penugasan aktif`;}applyAreaAssignmentPanelState();
}
async function toggleAreaAssignment(email,encodedArea,checked){
 const area=decodeURIComponent(encodedArea),map=getAreaAssignments(),user=USERS.find(u=>u.email===email);
 // Supervisor yang belum punya entry lokal dianggap memiliki semua area sebelum checkbox diubah.
 const base=(Array.isArray(map[email])?map[email]:((user?.role==='supervisor')?allAreas():[]));
 const set=new Set(base); checked?set.add(area):set.delete(area); map[email]=[...set];
 localStorage.setItem(AREA_ASSIGNMENT_KEY,JSON.stringify(map));
 renderAreaAssignments();
 try{
   const session=getSbSession();
   if(currentUser?.role==='admin'||isSupervisorUser(currentUser)){
     if(navigator.onLine&&session?.session_token){
       await rpc('app_admin_set_user_area_assignments',{p_token:session.session_token,p_sales_email:email,p_areas:map[email]||[]});
     }
   }
   markRemoteAreaAssignmentReady(email);
   toast("Penugasan area disimpan");
 }catch(e){
   console.error('Gagal menyimpan penugasan area:',e);
   toast(`Tersimpan di perangkat, sinkronisasi server gagal: ${e.message||'Periksa koneksi/SQL'}`);
 }
}
async function renameAreaName(encodedOldArea){
 if(currentUser?.role!=="admin")return;const oldArea=decodeURIComponent(encodedOldArea),input=prompt("Ubah nama area",oldArea);if(input===null)return;const newArea=String(input||"").trim();if(!newArea){toast("Nama area tidak boleh kosong");return}if(newArea===oldArea)return;if(allAreas().some(a=>a.toLowerCase()===newArea.toLowerCase())){toast("Nama area sudah digunakan");return}
 const rows=customers().map(c=>String(c.area||"").trim()===oldArea?{...c,area:newArea}:c);saveCustomers(rows,{sync:false});
 const assignments=getAreaAssignments();Object.keys(assignments).forEach(email=>{assignments[email]=[...new Set((assignments[email]||[]).map(a=>a===oldArea?newArea:a))]});localStorage.setItem(AREA_ASSIGNMENT_KEY,JSON.stringify(assignments));
 const products=productAssignments(),migrated={};Object.entries(products).forEach(([key,value])=>{const parts=key.split("@@");if(parts.length>=2&&parts[1]===oldArea)parts[1]=newArea;migrated[parts.join("@@")]=value});saveProductAssignmentsLocal(migrated);
 try{await Promise.all([syncCustomersToSupabase({silent:true}),syncSettingsToSupabase(),syncProductsToSupabase({silent:true})]);toast("Nama area berhasil diubah")}catch(e){console.error(e);toast("Nama area diubah lokal dan akan disinkronkan saat online")}
 fillAreas();fillHistorySalesFilter();renderAreaAssignments();
}
...
async function saveUserAccount(index){
 ...
 const requestedRole=(currentUser?.role==="admin"&&user.role!=="admin")?(document.getElementById(`userRole_${index}`)?.value||user.role):user.role;
 if(!["sales","supervisor"].includes(requestedRole)&&user.role!=="admin")return toast("Peran akun tidak valid");
...
async function resetUserPassword(index){
 if(!(currentUser?.role==="admin"||isSupervisorUser()))return;const user=USERS[index];if(!user)return;
...
function openOutsideAreaOrder(){
 ...
 const areas=supervisor?allAreas():allAreas().filter(a=>a!==getDailyArea());
 if(!areas.length)return toast("Tidak ada outlet yang tersedia");
 hide();
 document.getElementById("outsideAreaOrderView").classList.remove("hidden");
 document.getElementById("outsideOrderActiveArea").textContent=supervisor?"Semua Area":(getDailyArea()||"Belum dipilih");
 const areaWrap=document.getElementById("outsideOrderAreaWrap");
 const searchWrap=document.getElementById("outsideOrderSearchWrap");
 areaWrap?.classList.toggle("hidden",supervisor);
 searchWrap?.classList.toggle("hidden",!supervisor);
 outsideOrderArea.innerHTML=areas.map(a=>`<option>${esc(a)}</option>`).join("");
 const search=document.getElementById("outsideOrderSearch");if(search)search.value="";
...
const DATA_VERSION="v16.3-edit-area-outlet";
...
const KM2_KM14_AREA="Km.2 - Km.14";
const KM2_KM14_OUTLET_NAMES=new Set([
 "two r&s mart","akiu","ameng","haiti","lite","herwan","kang eng",
 "agus lubuk","suriyanto","herman km.8","bina setia","suheri",
 "weldy","witanto","edy km.14"
]);
function normalizeOutletName(value){return String(value||"").trim().toLowerCase().replace(/\s+/g," ")}
function applyKm2Km14AreaMigration(list){
 let changed=false;
 const rows=(Array.isArray(list)?list:[]).map(customer=>{
  if(normalizeOutletName(customer?.name)&&KM2_KM14_OUTLET_NAMES.has(normalizeOutletName(customer.name))&&String(customer.area||"").trim().toLowerCase()==="sawang"){
   changed=true;
   return {...customer,area:KM2_KM14_AREA};
  }
  return customer;
 });
 return {rows,changed};
...
function customers(){
 const parsed=JSON.parse(localStorage.getItem("rml_customers")||"[]");
 const migrated=applyKm2Km14AreaMigration(parsed);
 if(migrated.changed)localStorage.setItem("rml_customers",JSON.stringify(migrated.rows));
...
async function login(){
 ...
 try{
  ...
  if(currentUser?.role==="sales"){
   ...
   if(!synced&&!isRemoteAreaAssignmentReady(currentUser.email))ensureSalesAreaAssignment(currentUser.email);
  }else if(currentUser?.role==="admin"){
   // Admin boleh menyinkronkan seluruh pengaturan. Supervisor tidak boleh
   // melakukan full-sync karena assignment lokal yang lama dapat menimpa
   // assignment supervisor di server. Perubahan area supervisor disimpan
   // lewat RPC per-user pada toggleAreaAssignment().
   try{await syncSettingsToSupabase()}catch(e){console.warn("Sinkronisasi pengaturan admin gagal",e)}
  }
...
function openApp(){
 ...
 document.getElementById("historyRefreshBtn")?.classList.toggle("hidden",!canViewAllSalesHistory());
 document.getElementById("assignmentNavBtn")?.classList.toggle("hidden",!(currentUser.role==="admin"||isSupervisorUser()));
 document.getElementById("targetNavBtn")?.classList.add("hidden");
 document.getElementById("accountNavBtn")?.classList.remove("hidden");
 document.getElementById("outsideAreaOrderBtn")?.classList.toggle("hidden",!(currentUser.role==="sales"||isSupervisorUser()));
 fillAreas();(currentUser.role==="admin"||isSupervisorUser())?showDashboard():(getDailyArea()?showCustomers():showDailyAreaSelection());
}
...
async function refreshDashboard(){
 ...
 document.getElementById("dashboardPriceCard")?.classList.toggle("hidden",isAdmin);
 ...
 document.getElementById("dashboardSubtitle").textContent=isAdmin?"Ringkasan aktivitas seluruh sales hari ini":"Progres kunjungan hari ini";
 ...
 const dailyAreaCard=document.getElementById("dailyAreaProgressCard");
 statsCard.classList.remove("hidden");

 if(!isAdmin){
   dailyAreaCard?.classList.add("hidden");
   ...
   document.getElementById("salesStatsList").innerHTML=`<div class="sales-stat-card">
     ...
     ${isSupervisorUser()?`<div class="dashboard-action-row">${area?`<button class="primary compact" onclick="showCustomers()">Lanjutkan Area ${esc(area)}</button>`:`<button class="primary compact" onclick="showDailyAreaSelection()">Pilih Area Hari Ini</button>`}<button class="secondary compact" onclick="showDailyAreaSelection()">Ganti Area</button><button class="primary compact" onclick="openOutsideAreaOrder()">+ Order Luar Area</button></div>`:(area?`<button class="primary compact" onclick="showCustomers()">Lanjutkan Area ${esc(area)}</button>`:`<button class="primary compact" onclick="showDailyAreaSelection()">Pilih Area Hari Ini</button>`)}
   </div>`;
   return;
 }
...
function renderDailyAreaProgress(){
 ...
 const list=document.getElementById("dailyAreaProgressList");
 const dateEl=document.getElementById("dailyAreaProgressDate");
 ...
 todayVisits.forEach(v=>{
   ...
   byArea.get(area).add(email);
 });
 card.classList.remove("hidden");
 if(dateEl)dateEl.textContent=new Date().toLocaleDateString("id-ID",{day:"2-digit",month:"long",year:"numeric"});
 if(!byArea.size){
   list.innerHTML='<div class="daily-area-empty"><strong>Belum ada kunjungan hari ini</strong><span>Progress akan muncul otomatis setelah sales melakukan kunjungan ke suatu area.</span></div>';
   return;
 }
 const usersByEmail=new Map(USERS.map(u=>[String(u.email||"").toLowerCase(),u]));
 const groups=[...byArea.entries()].sort((a,b)=>a[0].localeCompare(b[0],"id",{numeric:true})).map(([area,emailSet])=>{
   const salesUsers=[...emailSet].map(email=>usersByEmail.get(email)).filter(u=>u&&(u.role==="sales"||u.role==="supervisor")&&u.active!==false);
...
function orderWithDeliveryHtml(visit,editable=false){
 ...
 const bulkControls=editable
   ? `<div class="delivery-bulk-bar">
        <span class="delivery-bulk-summary">Terkirim ${deliveredCount}/${items.length}</span>
        <div class="delivery-bulk-actions">
          <button type="button" class="btn btn-small" onclick="setAllDeliveryItems('${esc(visit.id)}',true,this)">Centang Semua</button>
          <button type="button" class="btn btn-small btn-secondary" onclick="setAllDeliveryItems('${esc(visit.id)}',false,this)">Hapus Semua</button>
        </div>
      </div>`
   : `<div class="delivery-bulk-summary readonly">Terkirim ${deliveredCount}/${items.length}</div>`;
 return `<div class="order-delivery-list">${bulkControls}${items.map((item,index)=>{
...
async function setAllDeliveryItems(visitId,delivered,buttonEl){
 if(!currentUser||currentUser.role!=="admin")return toast("Hanya admin yang dapat mengubah status pengiriman");
 const container=buttonEl?.closest?.(".order-delivery-list");
 const buttons=container?Array.from(container.querySelectorAll(".delivery-bulk-actions button")):[];
 buttons.forEach(btn=>btn.disabled=true);
 if(buttonEl)buttonEl.textContent=delivered?"Mencentang...":"Menghapus...";
 try{
   await refreshVisitCache();
   const visit=visitCache.find(v=>String(v.id)===String(visitId));
   if(!visit)throw new Error("Data kunjungan tidak ditemukan");
   const items=getDeliveryItems(visit).map(item=>({...item,delivered:Boolean(delivered)}));
   ...
   if(synced){
     await pullRemoteVisits();
     toast(delivered?"Semua barang ditandai sudah dikirim":"Semua centang pengiriman dihapus");
   }else{
     toast("Perubahan tersimpan di perangkat dan akan disinkronkan otomatis");
   }
   await refreshVisitCache();
   await renderHistory();
 }catch(e){
   console.error(e);
   toast(e?.message||"Gagal memperbarui semua status pengiriman");
 }finally{
   buttons.forEach(btn=>btn.disabled=false);
   if(buttonEl)buttonEl.textContent=delivered?"Centang Semua":"Hapus Semua";
 }
}
...
function updateEditVisitFields(){
 const status=document.getElementById("editVisitStatus")?.value||"";
 const label=document.getElementById("editVisitNoteLabel");
 const input=document.getElementById("editVisitNoteInput");
 if(!label||!input)return;
 if(status==="Ada Order"){
   label.textContent="Orderan *";
   input.placeholder="Masukkan orderan pelanggan...";
   enableOrderBulletInput(input,true);
 }else if(status==="Tidak Order"){
   label.textContent="Alasan Tidak Order *";
   input.placeholder="Tuliskan alasan tidak order...";
   enableOrderBulletInput(input,false);
 }else{
   label.textContent="Catatan Tidak Bertemu *";
   input.placeholder="Tuliskan catatan tidak bertemu...";
   enableOrderBulletInput(input,false);
 }
}
...
async function openVisitDetailModal(salesEmail,status){
 ...
 document.getElementById("visitDetailModalList").innerHTML=rows.map(v=>{
   ...
   return `<div class="visit-detail-item">
     ...
     ${canEditVisitNote(v)?`<button class="secondary compact edit-visit-button" onclick="openEditVisitModal('${esc(v.id)}')">Edit Kunjungan</button>`:""}
   </div>`;
...
async function openEditVisitModal(visitId){
 ...
 document.getElementById("editVisitStatus").value=visit.status||"Ada Order";
 document.getElementById("editVisitNoteInput").value=visit.status==="Ada Order"?normalizeOrderNote(visit.note):visit.note||"";
 document.querySelectorAll('input[name="editVisitPaymentStatus"]').forEach(el=>el.checked=Boolean(visit.paymentStatus&&el.value===visit.paymentStatus));
 updateEditVisitFields();
 ...
 document.getElementById("editVisitAuditList").innerHTML=audit.slice().reverse().map(item=>`<div class="edit-audit-item"><strong>${esc(item.editedByName||item.editedByEmail||"-")}</strong><span>${esc(formatVisitDateTime(item.editedAt))}</span><small>${esc(item.oldStatus||"-")} → ${esc(item.newStatus||"-")} • ${esc(item.oldPaymentStatus||"-")} → ${esc(item.newPaymentStatus||"-")}</small></div>`).join("");
 document.getElementById("editVisitNoteModal").classList.remove("hidden");
 setTimeout(()=>{const input=document.getElementById("editVisitNoteInput");input.focus();const end=input.value.length;input.setSelectionRange(end,end);input.scrollTop=input.scrollHeight;},80);
}
async function saveEditedVisitNote(){
 ...
 if(!["Ada Order","Tidak Order","Tidak Bertemu"].includes(status))return toast("Pilih status kunjungan");
 if(!paymentStatus)return toast("Pilih Bayar atau Tidak Bayar");
 const newNote=status==="Ada Order"?normalizeOrderNote(input.value):input.value.trim();
 const label=getVisitDetailFieldLabel(status);
 if(!newNote){toast(`${label} wajib diisi.`);input.focus();return;}
 if(status===visit.status&&newNote===String(visit.note||"").trim()&&paymentStatus===(visit.paymentStatus||"")){toast("Tidak ada perubahan yang disimpan");return;}
 const editedAt=new Date().toISOString();
 const change={oldStatus:visit.status||"",newStatus:status,oldNote:visit.note||"",newNote,oldPaymentStatus:visit.paymentStatus||"",newPaymentStatus:paymentStatus,editedAt,editedByEmail:currentUser.email,editedByName:currentUser.name,editorRole:currentUser.role};
 const updated={...visit,status,note:newNote,paymentStatus,deliveryItems:status==="Ada Order"?orderLines(newNote).map(text=>{const old=(Array.isArray(visit.deliveryItems)?visit.deliveryItems:[]).find(x=>String(x.text||"").trim()===String(text).trim());return {text,delivered:Boolean(old?.delivered)};}):[],editHistory:[...(Array.isArray(visit.editHistory)?visit.editHistory:[]),change],lastEditedAt:editedAt,lastEditedBy:currentUser.email};
 const btn=document.getElementById("saveEditVisitButton");if(btn){btn.disabled=true;btn.textContent="Menyimpan...";}
 try{
   await saveVisitOffline(updated);await refreshVisitCache();closeEditVisitModal();toast("Kunjungan berhasil diperbarui");
   if(!document.getElementById("visitDetailModal").classList.contains("hidden"))await openVisitDetailModal(currentVisitDetailSalesEmail,currentVisitDetailStatus);
   if(!document.getElementById("historyView").classList.contains("hidden"))await renderHistory();
   if(!document.getElementById("dashboardView").classList.contains("hidden"))await refreshDashboard();
...
function getDailyArea(){
 if(!currentUser||!(currentUser.role==="sales"||isSupervisorUser()))return "";
...
async function chooseArea(area){
 ...
 showCustomers();
 setTimeout(()=>showDueReminder(area),180);
}

function dueReminderOutlets(area){
 ...
 return customers().filter(c=>{
   ...
   if(c.dueDate){
     const due=new Date(`${c.dueDate}T00:00:00`);
     if(!Number.isNaN(due.getTime()))days=Math.ceil((due-today)/86400000);
   }
   return {...c,days};
 }).sort((a,b)=>{
...
function dueReminderStatus(days){
 if(days===null)return "Tanggal belum diisi";
 if(days<0)return `Lewat ${Math.abs(days)} hari`;
 if(days===0)return "Jatuh tempo hari ini";
 return `${days} hari lagi`;
}
function showDueReminder(area){
 const rows=dueReminderOutlets(area);
 if(!rows.length)return;
 const modal=document.getElementById("dueReminderModal");
 document.getElementById("dueReminderTitle").textContent=`${rows.length} Outlet Perlu Diingatkan`;
 document.getElementById("dueReminderSubtitle").textContent=`Hari ini di area ${area} ada ${rows.length} outlet yang jatuh tempo dalam 2 bulan atau sudah melewati jatuh tempo.`;
 document.getElementById("dueReminderList").innerHTML=rows.map(c=>{
   const statusClass=c.days!==null&&c.days<0?"overdue":c.days!==null&&c.days<=14?"urgent":"warning";
   return `<button class="due-reminder-item ${statusClass}" onclick="openDueReminderOutlet('${esc(c.no)}')">
     <div class="due-reminder-item-main">
       <strong>${esc(c.name)}</strong>
       <span>${esc(c.code||"TANPA KODE")} • ${esc(c.area)}</span>
       ${c.dueAmount?`<span>Tagihan: ${formatRupiah(c.dueAmount)}</span>`:""}
     </div>
     <div class="due-reminder-date">
       <b>${esc(dueReminderStatus(c.days))}</b>
       <span>${c.dueDate?esc(formatDate(c.dueDate)):"Belum ada tanggal"}</span>
     </div>
   </button>`;
 }).join("");
...
function closeDueReminder(){
 document.getElementById("dueReminderModal")?.classList.add("hidden");
 document.body.classList.remove("modal-open");
}
function openDueReminderOutlet(customerNo){
 closeDueReminder();
...
async function showCustomers(){
 await refreshVisitCache();
 if(currentUser&&currentUser.role!=="admin"&&!getDailyArea())return showDailyAreaSelection();
 hide();
   ...
   <div class="admin-outlet-status-row">
    <label>Status Outlet
     <select onchange="setCustomerOutletStatus(${x.no},this.value)">
      <option value="normal" ${(x.outletStatus||"normal")==="normal"?"selected":""}>Normal</option>
      <option value="dueSoon" ${x.outletStatus==="dueSoon"?"selected":""}>Jatuh Tempo Dekat</option>
      <option value="pending" ${x.outletStatus==="pending"?"selected":""}>Pending Barang</option>
     </select>
    </label>
    <span class="badge ${getOutletStatusClass(x.outletStatus)}">${esc(getOutletStatusLabel(x.outletStatus))}</span>
   </div>
   <div class="admin-customer-actions">
    <button class="secondary compact admin-edit-area" type="button" onclick="openCustomerAreaEditor(${x.no})">Edit Area</button>
...
async function setCustomerOutletStatus(no,status){
 if(currentUser?.role!=="admin")return;
 const allowed=["normal","dueSoon","pending"];
 if(!allowed.includes(status))return;
 const data=customers(),i=data.findIndex(x=>Number(x.no)===Number(no));
 if(i<0)return;
 const previous={...data[i]};
 data[i]={...data[i],outletStatus:status};
 if(status==="normal"){data[i].dueDate="";data[i].dueAmount=0;data[i].outletNote=""}
 if(status==="pending"){data[i].dueAmount=0}
 saveCustomers(data,{sync:false});renderCustomers();
 try{
  const synced=await syncCustomersToSupabase();
  if(!synced&&navigator.onLine)throw new Error("Sesi sinkronisasi tidak tersedia");
  toast(`Status outlet diubah menjadi ${getOutletStatusLabel(status)}`);

 }catch(e){
  data[i]=previous;saveCustomers(data,{sync:false});renderCustomers();toast("Gagal menyimpan status outlet");
 }
}
...
function ensureCustomerAreaModal(){
 ...
 modal.id="customerAreaEditModal";
 modal.className="customer-area-modal hidden";
 modal.innerHTML=`<div class="customer-area-dialog" role="dialog" aria-modal="true" aria-labelledby="customerAreaModalTitle">
  <div class="customer-area-dialog-head"><div><small>Edit Area Pelanggan</small><h3 id="customerAreaModalTitle">Pilih Area</h3></div><button type="button" class="icon-close" aria-label="Tutup" onclick="closeCustomerAreaEditor()">×</button></div>
  <div class="customer-area-dialog-body">
   <div class="customer-area-outlet"><small>Pelanggan</small><strong id="customerAreaOutletName">-</strong></div>
   <label>Area<select id="customerAreaSelect"></select></label>
   <label id="customerAreaCustomWrap" class="hidden">Nama Area Baru<input id="customerAreaCustomInput" type="text" placeholder="Masukkan nama area"></label>
  </div>
  <div class="customer-area-dialog-actions"><button type="button" class="secondary" onclick="closeCustomerAreaEditor()">Batal</button><button type="button" id="customerAreaSaveBtn" onclick="saveCustomerAreaEdit()">Simpan Area</button></div>
 </div>`;
 modal.addEventListener("click",event=>{if(event.target===modal)closeCustomerAreaEditor()});
 document.body.appendChild(modal);
 document.getElementById("customerAreaSelect").addEventListener("change",event=>{
  const custom=event.target.value==="__CUSTOM__";
  document.getElementById("customerAreaCustomWrap").classList.toggle("hidden",!custom);
  if(custom)setTimeout(()=>document.getElementById("customerAreaCustomInput")?.focus(),0);
 });
 return modal;
}
let customerAreaEditingNo=null;
function openCustomerAreaEditor(no){
 if(currentUser?.role!=="admin")return;
 const customer=customers().find(x=>String(x.no)===String(no));
 if(!customer)return toast("Pelanggan tidak ditemukan");
 customerAreaEditingNo=customer.no;
 const modal=ensureCustomerAreaModal();
 const areas=[...new Set([...allAreas(),KM2_KM14_AREA,customer.area].filter(Boolean))].sort((a,b)=>a.localeCompare(b,"id",{numeric:true,sensitivity:"base"}));
 const select=document.getElementById("customerAreaSelect");
 select.innerHTML=areas.map(area=>`<option value="${esc(area)}">${esc(area)}</option>`).join("")+`<option value="__CUSTOM__">+ Tambah Area Baru</option>`;
 select.value=areas.includes(customer.area)?customer.area:"__CUSTOM__";
 document.getElementById("customerAreaOutletName").textContent=`${customer.code||"Tanpa Kode"} • ${customer.name}`;
 const customWrap=document.getElementById("customerAreaCustomWrap"),customInput=document.getElementById("customerAreaCustomInput");
 customWrap.classList.toggle("hidden",select.value!=="__CUSTOM__");
 customInput.value=select.value==="__CUSTOM__"?(customer.area||""):"";
 modal.classList.remove("hidden");
 document.body.classList.add("modal-open");
}
function closeCustomerAreaEditor(){
 document.getElementById("customerAreaEditModal")?.classList.add("hidden");
 document.body.classList.remove("modal-open");
 customerAreaEditingNo=null;
}
async function saveCustomerAreaEdit(){
 if(currentUser?.role!=="admin"||customerAreaEditingNo===null)return;
 const select=document.getElementById("customerAreaSelect"),custom=document.getElementById("customerAreaCustomInput");
 const nextArea=(select.value==="__CUSTOM__"?custom.value:select.value).trim();
 if(!nextArea)return toast("Area wajib diisi");
 const data=customers(),i=data.findIndex(x=>String(x.no)===String(customerAreaEditingNo));
 if(i<0)return toast("Pelanggan tidak ditemukan");
 const previous={...data[i]},button=document.getElementById("customerAreaSaveBtn"),oldText=button.textContent;
 if(String(previous.area||"")===nextArea){closeCustomerAreaEditor();return}
 button.disabled=true;button.textContent="Menyimpan...";
 data[i]={...data[i],area:nextArea};
 saveCustomers(data,{sync:false});fillAreas();renderCustomers();
 try{
  if(!navigator.onLine){scheduleCustomerSync();closeCustomerAreaEditor();toast("Area diubah. Akan disinkronkan saat online.");return}
  const ok=await syncCustomersToSupabase({silent:true});
  if(!ok)throw new Error("Sinkronisasi Supabase gagal");
  closeCustomerAreaEditor();toast("Area pelanggan berhasil diubah dan disinkronkan");
 }catch(error){
  data[i]=previous;saveCustomers(data,{sync:false});fillAreas();renderCustomers();
  toast(`Gagal mengubah area: ${error.message||"Periksa koneksi"}`);
...
function renderCustomers(){
 ...
 if(currentUser.role!=="admin"){
  ...
  base=all.filter(x=>!x.isHidden&&(isSupervisorUser()||(isAreaAssigned(currentUser.email,x.area)&&canSalesAccessCustomer(x,currentUser.email)))&&x.area===getDailyArea());
...
function selectCustomer(no){
 ...
 if(!selected)return;
 if(currentUser.role==="sales"&&!isSupervisorUser()&&selected.isHidden)return toast("Pelanggan ini tidak aktif");
 if(currentUser.role==="sales"&&!isSupervisorUser()&&!canSalesAccessCustomer(selected,currentUser.email))
...
function showCheckOutPanel(active){
 ...
 const priceCard=document.getElementById("visitPriceSearchCard");if(priceCard)priceCard.open=false;
 const priceInput=document.getElementById("visitPriceSearchInput");if(priceInput)priceInput.value="";
 renderVisitPriceSearch();
 updateVisitNoteField();
...
function updateVisitNoteField(){
 ...
 if(status==="Ada Order"){
   ...
   note.rows=8;
   note.classList.add("visit-order-textarea");
   enableOrderBulletInput(note,true);
 }else if(status==="Tidak Order"){
   note.rows=4;
   note.classList.remove("visit-order-textarea");
   label.textContent="Alasan Tidak Order *";
...
function handleVisitPriceSearchToggle(details){
 if(!details?.open)return;
 renderVisitPriceSearch();
 setTimeout(()=>document.getElementById("visitPriceSearchInput")?.focus(),80);
}
function renderVisitPriceSearch(){
 const input=document.getElementById("visitPriceSearchInput");
 const box=document.getElementById("visitPriceSearchResults");
 const summary=document.getElementById("visitPriceSearchSummary");
 if(!input||!box||!summary)return;
 const q=normalizeProductSearchText(input.value||"");
 if(!q){box.innerHTML="";summary.textContent="Ketik nama atau kode barang.";return;}
 const rows=productCatalog().filter(x=>x.active!==false&&normalizeProductSearchText(`${x.code||""} ${x.name||""} ${x.category||""}`).includes(q))
  .sort((a,b)=>String(a.name||"").localeCompare(String(b.name||""),"id")).slice(0,20);
 summary.textContent=rows.length?`${rows.length} hasil teratas`:`Barang tidak ditemukan`;
 box.innerHTML=rows.map(x=>{const discount=normalizeProductDiscount(x.discount),net=productNetPrice(x);return `<article class="visit-price-result"><div><small>${esc(x.category||"Tanpa Kategori")} · ${esc(x.code||"Tanpa Kode")}</small><strong>${esc(x.name||"Produk")}</strong></div><div class="visit-price-result-price"><b>${formatProductPrice(x.price)}</b>${discount>0?`<small>Diskon ${formatProductDiscount(discount)} · Netto ${formatProductPrice(net)}</small>`:""}</div></article>`}).join("");
}
...
async function refreshHistoryFromServer(){
 if(!currentUser||!canViewAllSalesHistory())return;
...
function showHistory(){
 ...
 const isAdmin=currentUser?.role==="admin";
 const canViewAll=canViewAllSalesHistory();
 document.getElementById("adminHistorySalesFilterWrap")?.classList.remove("hidden");
 document.getElementById("historySalesField")?.classList.toggle("hidden",!canViewAll);
 document.getElementById("historyAreaField")?.classList.toggle("hidden",!canViewAll);
 document.getElementById("historyOutletField")?.classList.toggle("hidden",!canViewAll);
 document.getElementById("historyVisibilityField")?.classList.toggle("hidden",!isAdmin);
...
function getHistoryPeriodRange(){
 ...
 let from=null,to=endOfLocalDay(now),label="Hari Ini";
 if(period==="today"){from=startOfLocalDay(now);label="Hari Ini"}
 ...
 else if(period==="7days"){const d=new Date(now);d.setDate(d.getDate()-6);from=startOfLocalDay(d);label="7 Hari Terakhir"}
 else if(period==="30days"){const d=new Date(now);d.setDate(d.getDate()-29);from=startOfLocalDay(d);label="30 Hari Terakhir"}
 return {period,from,to,label};
}
function handleHistoryPeriodChange(){
 historyRenderLimit=HISTORY_PAGE_SIZE;
 document.getElementById("historyCustomDateWrap")?.classList.add("hidden");
 renderHistory();
}
...
function refreshHistoryFilterOptions(allRows){
 if(!canViewAllSalesHistory())return;
...
function isVisitAutoHidden(visit){
 if(!visit)return false;
 if(visit.status==="Tidak Order"||visit.status==="Tidak Bertemu")return true;
 if(visit.status!=="Ada Order")return false;
 const items=getDeliveryItems(visit);
 return items.length>0&&items.every(item=>Boolean(item.delivered));
}
function handleHistoryVisibilityChange(){
 historyRenderLimit=HISTORY_PAGE_SIZE;
 expandedHistoryOutletKey="";
 renderHistory();
}
function getHistoryOutletKey(visit){
 return String(visit.customerNo||visit.code||`${visit.name||"Tanpa Nama"}|${visit.area||""}`);
}
function toggleHistoryOutlet(encodedKey){
 const key=decodeURIComponent(String(encodedKey||""));
 expandedHistoryOutletKey=expandedHistoryOutletKey===key?"":key;
 renderHistory();
}
function summarizeOutletVisits(rows){
 const countStatus=status=>rows.filter(v=>v.status===status).length;
 const deliveredRows=rows.filter(isVisitAutoHidden).length;
 return {
  total:rows.length,
  adaOrder:countStatus("Ada Order"),
  tidakOrder:countStatus("Tidak Order"),
  tidakBertemu:countStatus("Tidak Bertemu"),
  deliveredRows
 };
}
function applyHistoryFilters(data){
 let rows=[...data];
 if(currentUser.role==="sales"&&!isSupervisorUser())rows=rows.filter(x=>x.salesEmail===currentUser.email);
 else{
 ...
 if(currentUser?.role==="admin"){
   const visibility=document.getElementById("historyVisibilityFilter")?.value||"active";
   if(visibility==="active")rows=rows.filter(v=>!isVisitAutoHidden(v));
   else if(visibility==="hidden")rows=rows.filter(isVisitAutoHidden);
 }
...
async function renderHistory(){
 ...
 const data=applyHistoryFilters(allRows);
 const range=getHistoryPeriodRange();
 const visibility=currentUser?.role==="admin"?(document.getElementById("historyVisibilityFilter")?.value||"active"):"all";
 const visibilityLabel=visibility==="hidden"?"Hidden":visibility==="all"?"Semua":"Aktif";
 const grouped=new Map();
 data.forEach(v=>{
   const key=getHistoryOutletKey(v);
   if(!grouped.has(key))grouped.set(key,[]);
   grouped.get(key).push(v);
 });
 const groups=[...grouped.entries()].sort((a,b)=>{
   const ad=visitDateValue(a[1][0])?.getTime()||0,bd=visitDateValue(b[1][0])?.getTime()||0;
   return bd-ad;
 });
 const summary=document.getElementById("historyFilterSummary");
 if(summary)summary.textContent=`${groups.length} outlet • ${data.length} kunjungan • ${range.label}${currentUser?.role==="admin"?` • ${visibilityLabel}`:""}`;
 updateHistoryDeleteToolbar();
 const visibleGroups=groups.slice(0,historyRenderLimit);
 const list=document.getElementById("historyList");
 if(!list)return;
 if(!groups.length){
   list.innerHTML='<div class="empty">Belum ada riwayat untuk filter yang dipilih.</div>';
   return;
 }
 if(expandedHistoryOutletKey&&!grouped.has(expandedHistoryOutletKey))expandedHistoryOutletKey="";
 list.innerHTML=visibleGroups.map(([key,rows])=>{
   const first=rows[0],stats=summarizeOutletVisits(rows),expanded=expandedHistoryOutletKey===key;
   const encoded=encodeURIComponent(key);
   const detailHtml=rows.map(v=>`
    <div class="history-card ${selectedVisitIds.has(String(v.id))?"selected-for-delete":""}" data-visit-id="${esc(v.id)}">
      ...
      <div class="history-visit-heading"><span>${esc(formatVisitDateTime(v.checkInAt||v.createdAt))}</span><span class="pill">${esc(v.status||"-")}</span></div>
      <div class="pills">
   ...
   return `<section class="history-outlet-group ${expanded?"expanded":"collapsed"}">
     <button class="history-outlet-header" type="button" onclick="toggleHistoryOutlet('${encoded}')" aria-expanded="${expanded}">
       <span class="history-outlet-chevron">${expanded?"⌄":"›"}</span>
       <span class="history-outlet-title"><small>${esc(first.code||"TANPA KODE")} • ${esc(first.area||"-")}</small><strong>${esc(first.name||"Tanpa Nama")}</strong></span>
       <span class="history-outlet-summary"><b>${stats.total}</b> kunjungan${stats.adaOrder?` • ${stats.adaOrder} order`:""}${currentUser?.role==="admin"&&stats.deliveredRows?` • ${stats.deliveredRows} selesai`:""}</span>
     </button>
     <div class="history-outlet-content ${expanded?"":"hidden"}">${detailHtml}</div>
   </section>`;
 }).join("")+(groups.length>visibleGroups.length?`<button class="secondary history-load-more" type="button" onclick="loadMoreHistory()">Muat ${Math.min(HISTORY_PAGE_SIZE,groups.length-visibleGroups.length)} outlet berikutnya</button>`:"");
}
...
window.addEventListener("online",()=>{updateNetworkStatus();updatePendingSyncCount();scheduleAutoSync(100);if(currentUser){
