...
function normalizeTargetSelectKey(v){return String(v??'').trim().toLowerCase()}
function setTargetSelectValue(el,value,aliases=[]){
 if(!el)return false;
 const wanted=[value,...aliases].map(normalizeTargetSelectKey).filter(Boolean);
 let opt=[...el.options].find(o=>wanted.includes(normalizeTargetSelectKey(o.value)));
 if(!opt)opt=[...el.options].find(o=>wanted.includes(normalizeTargetSelectKey(o.textContent)));
 if(!opt){
  const compact=wanted.map(v=>v.replace(/\s*[—-]\s*.*/,''));
  opt=[...el.options].find(o=>compact.includes(normalizeTargetSelectKey(o.value).replace(/\s*[—-]\s*.*/,'')) || compact.includes(normalizeTargetSelectKey(o.textContent).replace(/\s*[—-]\s*.*/,'')));
 }
 if(opt){el.value=opt.value;el.dispatchEvent(new Event('change',{bubbles:false}));return true}
 return false;
}
function fillTargetManagerFields(preserveValues={}){
 ...
 if(!owner)return;
 const oldOwner=preserveValues.ownerId ?? owner.value;
 const oldSales=preserveValues.targetSalesEmail ?? sales?.value ?? '';
 ...
 if(type==='sales'){
  ...
  setTargetSelectValue(owner,oldOwner,[salesName(oldOwner),usersList().find(u=>normalizeTargetSelectKey(u.email)===normalizeTargetSelectKey(oldOwner))?.name]);
 }else{
  ...
  const outlet=customersList().find(c=>[c.no,c.id,c.code,c.name].map(normalizeTargetSelectKey).includes(normalizeTargetSelectKey(oldOwner)));
  setTargetSelectValue(owner,oldOwner,outlet?[outlet.no,outlet.id,outlet.code,outlet.name]:[]);
  const user=selectableUsers.find(u=>[u.email,u.account_key,u.login,u.username,u.name].map(normalizeTargetSelectKey).includes(normalizeTargetSelectKey(oldSales)));
  setTargetSelectValue(sales,oldSales,user?[user.email,user.account_key,user.login,user.username,user.name]:[]);
 }
}
...
function resetTargetForm(){document.getElementById('targetEditId').value='';document.getElementById('targetTypeInput').value='sales';document.getElementById('targetMonthInput').value=monthNow();document.getElementById('targetDurationInput').value='1';const amount=document.getElementById('targetAmountInput');if(amount)amount.value='';document.getElementById('targetAchievedInput').value='';document.getElementById('targetRewardTypeInput').value='nominal';document.getElementById('targetRewardValueInput').value='';renderTargetTiersForm([{target:'',rewardType:'nominal',rewardValue:''}]);document.getElementById('targetDescriptionInput').value='';document.getElementById('targetUpdatedDateInput').value=new Date().toISOString().slice(0,10);fillTargetManagerFields();refreshTargetRewardFields()}
...
async function editDashboardTarget(id){if(!canManageTargets())return toast('Tidak memiliki izin mengedit target');await pullTargetsFromServer({silent:false});const x=targetRows().find(r=>String(r.id)===String(id));if(!x)return toast('Target tidak ditemukan di server');document.getElementById('targetManagerModal')?.classList.remove('hidden');document.getElementById('targetEditId').value=x.id;document.getElementById('targetTypeInput').value=x.type;document.getElementById('targetMonthInput').value=targetStart(x);document.getElementById('targetDurationInput').value=periodMonths(x);const amount=document.getElementById('targetAmountInput');if(amount)amount.value=x.target||'';document.getElementById('targetAchievedInput').value=x.achieved||0;document.getElementById('targetRewardTypeInput').value=x.rewardType||'none';document.getElementById('targetRewardValueInput').value=x.rewardValue||'';document.getElementById('targetDescriptionInput').value=x.description||'';setTargetTiersForm(x.type==='sales'?(x.tiers&&x.tiers.length?x.tiers:[{target:x.target,rewardType:x.rewardType||'nominal',rewardValue:x.rewardValue||0}]):[]);document.getElementById('targetUpdatedDateInput').value=x.updatedDate||x.updatedAt?.slice?.(0,10)||new Date().toISOString().slice(0,10);fillTargetManagerFields({ownerId:x.ownerId,targetSalesEmail:x.targetSalesEmail||''});refreshTargetRewardFields();await renderTargetManagerList()}
