...
function editProduct(id){const x=productCatalog().find(p=>String(p.id)===String(id));if(!x)return;document.getElementById('productEditId').value=x.id;document.getElementById('productCodeInput').value=x.code||'';document.getElementById('productNameInput').value=x.name||'';document.getElementById('productCategoryInput').value=x.category||'';document.getElementById('productPriceInput').value=Number(x.price)||0;document.getElementById('productActiveInput').checked=x.active!==false;document.getElementById('productNameInput').scrollIntoView({behavior:'smooth',block:'center'})}
async function saveProductFromForm(){
 if(currentUser?.role!=='admin')return;
 const id=document.getElementById('productEditId').value||makeProductId();const name=document.getElementById('productNameInput').value.trim();const category=document.getElementById('productCategoryInput').value.trim();const price=Number(document.getElementById('productPriceInput').value);if(!name||!category||!Number.isFinite(price)||price<0)return toast('Nama, kategori, dan harga wajib diisi');
 const rows=productCatalog(),i=rows.findIndex(x=>String(x.id)===String(id));const item={id,code:document.getElementById('productCodeInput').value.trim(),name,category,price:Math.round(price),active:document.getElementById('productActiveInput').checked};if(i>=0)rows[i]={...rows[i],...item};else rows.push(item);saveProductCatalogLocal(rows);clearProductForm();fillProductFilters();renderPriceList();renderProductAssignments();
 const ok=navigator.onLine?await syncProductsToSupabase({silent:true}):false;toast(ok?'Barang berhasil disimpan dan disinkronkan':'Barang tersimpan di perangkat dan akan disinkronkan saat online');if(!ok)scheduleProductSync();
}
...
function renderProductAssignments(){
 if(currentUser?.role!=='admin')return;const email=document.getElementById('productSalesFilter')?.value;if(!email){document.getElementById('productAssignmentList').innerHTML='<div class="empty">Belum ada akun Sales.</div>';return}const cat=document.getElementById('productAssignCategoryFilter')?.value||'';const selected=new Set(productAssignments()[email]||[]);const rows=productCatalog().filter(x=>x.active!==false&&(!cat||x.category===cat)).sort((a,b)=>String(a.name).localeCompare(String(b.name),'id'));
 document.getElementById('productAssignmentList').innerHTML=rows.map(x=>`<label class="product-assignment-item"><input type="checkbox" ${selected.has(String(x.id))?'checked':''} onchange="toggleProductAssignment('${esc(email)}','${esc(x.id)}',this.checked)"><span><strong>${esc(x.name)}</strong><small>${esc(x.category)} · ${formatProductPrice(x.price)}</small></span></label>`).join('')||'<div class="empty">Tidak ada barang pada kategori ini.</div>';
}
...
async function setAllVisibleProductAssignments(checked){const email=document.getElementById('productSalesFilter')?.value;if(!email)return;const cat=document.getElementById('productAssignCategoryFilter')?.value||'';const visible=productCatalog().filter(x=>x.active!==false&&(!cat||x.category===cat)).map(x=>String(x.id));const map=productAssignments(),set=new Set(map[email]||[]);visible.forEach(id=>checked?set.add(id):set.delete(id));map[email]=[...set];saveProductAssignmentsLocal(map);renderProductAssignments();const ok=navigator.onLine?await syncProductsToSupabase({silent:true}):false;toast(ok?'Pembagian barang berhasil disimpan':'Pembagian tersimpan lokal dan akan disinkronkan saat online');if(!ok)scheduleProductSync()}
