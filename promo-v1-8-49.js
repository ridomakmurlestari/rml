...
(function(){
  ...
  const normalizeCategories=raw=>{
    if(raw&&typeof raw==='object'&&!Array.isArray(raw)&&Array.isArray(raw.categories)) return raw.categories.map((c,i)=>({name:String(c?.name||`Kategori ${i+1}`).trim(),items:Array.isArray(c?.items)?c.items.map(x=>String(x).trim()).filter(Boolean):[]})).filter(c=>c.name||c.items.length);
    if(Array.isArray(raw)){const old=raw.map(x=>String(x).trim()).filter(Boolean);return old.length?[{name:'Umum',items:old}]:[]}
    return [];
  };
  ...
  async function pullRemote(force=false){
    const now=Date.now();
    if(!force&&lastPullAt&&now-lastPullAt<120000)return true;
    hydrateFromLocal();
    ...
    try{
      ...
      const all=readLocal(),m={}; remoteRows.forEach(r=>{m[r.ownerEmail]={categories:r.categories}}); all[monthKey()]=m; writeLocal(all);
      lastPullAt=Date.now(); localStorage.setItem(CACHE_KEY,JSON.stringify({monthKey:monthKey(),at:lastPullAt}));
      return true;
  ...
  function signature(c){return `${String(c.name||'').trim().toLowerCase()}\n${(c.items||[]).map(x=>String(x).trim()).filter(Boolean).join('\n').toLowerCase()}`}
  function buildMasterCards(){
    const map=new Map();
    remoteRows.forEach(r=>r.categories.forEach(c=>{if(!c.name&&!c.items.length)return;const key=signature(c);let card=map.get(key);if(!card){card={name:c.name,items:[...c.items],assignees:[]};map.set(key,card)}if(!card.assignees.includes(r.ownerEmail))card.assignees.push(r.ownerEmail)}));
    masterCards=[...map.values()];
  }
  function escAttr(v){return escP(v).replace(/`/g,'&#96;')}
  function renderAssigneeChecks(selected=[]){const box=document.getElementById('promoAssignees');if(!box)return;const sel=new Set(selected.map(x=>String(x).toLowerCase()));box.innerHTML=allAssignees().map(u=>{const e=String(u.email||'').toLowerCase();return `<label class="promo-assignee"><input type="checkbox" value="${escAttr(e)}" ${sel.has(e)?'checked':''}><span>${escP(u.name||e)}${u.role==='supervisor'?' — Supervisor':''}</span></label>`}).join('')||'<div class="empty">Belum ada Sales/Supervisor aktif.</div>'}
  function renderMasterCards(){
    ...
    host.innerHTML=masterCards.map((c,i)=>`<article class="promo-master-card" data-promo-index="${i}"><div class="promo-master-head"><div><span class="promo-master-icon">🎁</span><div><strong>${escP(c.name||'Promo')}</strong><small>${c.items.length} barang • ${c.assignees.length} penanggung jawab</small></div></div><button class="danger compact" type="button" onclick="removeMasterPromo(${i})">Hapus</button></div><label>Kategori Promo</label><input class="promo-master-name" type="text" value="${escAttr(c.name||'')}" placeholder="Contoh: Dodo"><label>Barang Promo</label><textarea class="promo-master-items" rows="5" placeholder="Satu baris untuk satu barang promo">${escP(c.items.join('\n'))}</textarea><label>Assign ke</label><div class="promo-assignees">${allAssignees().map(u=>{const e=String(u.email||'').toLowerCase();return `<label class="promo-assignee"><input type="checkbox" value="${escAttr(e)}" ${c.assignees.includes(e)?'checked':''}><span>${escP(u.name||e)}${u.role==='supervisor'?' — Supervisor':''}</span></label>`}).join('')}</div></article>`).join('');
  }
  function addMasterPromo(){masterCards.push({name:'',items:[],assignees:[]});renderMasterCards();setTimeout(()=>{const cards=document.querySelectorAll('.promo-master-card');cards[cards.length-1]?.scrollIntoView({behavior:'smooth',block:'center'});cards[cards.length-1]?.querySelector('.promo-master-name')?.focus()},30)}
  function readMasterCards(){return [...document.querySelectorAll('.promo-master-card')].map(el=>({name:String(el.querySelector('.promo-master-name')?.value||'').trim(),items:String(el.querySelector('.promo-master-items')?.value||'').split(/\r?\n/).map(x=>x.trim()).filter(Boolean),assignees:[...el.querySelectorAll('.promo-assignees input[type=checkbox]:checked')].map(x=>x.value.toLowerCase())})).filter(c=>c.name||c.items.length||c.assignees.length)}
  ...
  async function savePage(){
    if(!canManage())return;
    const cards=readMasterCards();
    if(!cards.length)return toast('Tambah minimal satu promo');
    if(cards.some(c=>!c.name))return toast('Nama kategori promo wajib diisi');
    if(cards.some(c=>!c.items.length))return toast('Setiap promo harus memiliki minimal satu barang');
    if(cards.some(c=>!c.assignees.length))return toast('Assign minimal satu Sales/Supervisor pada setiap promo');
    ...
    const oldOwners=new Set(remoteRows.map(r=>r.ownerEmail));
    Object.keys(byOwner).forEach(e=>oldOwners.delete(e));
    try{
      for(const [email,cats] of Object.entries(byOwner)) await rpc('app_admin_upsert_monthly_promo',{p_token:token,p_month_key:monthKey(),p_sales_email:email,p_items:{categories:cats}});
      for(const email of oldOwners) await rpc('app_admin_upsert_monthly_promo',{p_token:token,p_month_key:monthKey(),p_sales_email:email,p_items:{categories:[]}});
      await pullRemote(true);buildMasterCards();renderMasterCards();renderSaved();renderCard();toast('Promo berhasil disimpan');
    }catch(e){toast(`Gagal menyimpan promo: ${e.message||'periksa koneksi'}`)}
  }
  async function removeMasterPromo(i){
  ...
  function salesPromoCard(email){const cats=cardsForOwner(email),items=cats.flatMap(c=>c.items||[]);if(!items.length)return '';const name=userName(email);return `<button class="dashboard-price-card monthly-promo-card monthly-promo-sales-card" type="button" onclick="openMonthlyPromoDetail('${escAttr(email)}')"><span class="dashboard-price-icon monthly-promo-icon" aria-hidden="true">🎁</span><span><strong>Promo ${escP(name)}</strong><small>${cats.length} kategori • ${items.length} barang • ${monthLabel(monthKey())}</small>${cats.slice(0,2).map(c=>`<span class="monthly-promo-category"><b>${escP(c.name)}</b><em>${escP(c.items.slice(0,2).join(' • '))}</em></span>`).join('')}</span><span class="dashboard-price-arrow">›</span></button>`}
  ...
  window.showPromoManagementPage=showPage;window.closePromoManagementPage=()=>{if(typeof showAreaAssignments==='function')showAreaAssignments();else if(typeof showDashboard==='function')showDashboard()};window.saveMonthlyPromo=savePage;window.addMasterPromo=addMasterPromo;window.removeMasterPromo=removeMasterPromo;window.openMonthlyPromoDetail=openDetail;window.closePromoDetailModal=closeDetail;window.closePromoCatalog=closeDetail;window.renderMonthlyPromoCard=renderCard;window.pullMonthlyPromos=pullRemote;
