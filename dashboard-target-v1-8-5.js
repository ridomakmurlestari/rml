/* RML Sales Visit v1.8.29 - Target Category Grouping / Group by Sales / Refresh Fix */
(function(){
...
const usersList=()=>{try{const saved=JSON.parse(localStorage.getItem('rml_user_accounts_v1')||'null');if(Array.isArray(saved)&&saved.length)return saved}catch(_){}return DEFAULT_TARGET_USERS};
...
function normalizeRemoteRow(r){return {id:String(r.id),type:r.type,startMonth:r.start_month||r.month||monthNow(),month:r.start_month||r.month||monthNow(),durationMonths:Number(r.duration_months||1),ownerId:r.owner_id||'',targetSalesEmail:r.target_sales_email||'',description:r.description||'',target:Number(r.target||0),achieved:Number(r.achieved||0),updatedDate:r.updated_date||'',updatedAt:r.updated_at||''}}
...
async function saveDashboardTarget(){if(!canManageTargets())return toast('Hanya Admin/Supervisor yang dapat menyimpan target');const type=document.getElementById('targetTypeInput').value,startMonth=document.getElementById('targetMonthInput').value,durationMonths=Number(document.getElementById('targetDurationInput').value||1),ownerId=document.getElementById('targetOwnerInput').value,description=document.getElementById('targetDescriptionInput').value.trim(),target=Number(document.getElementById('targetAmountInput').value||0),achieved=Number(document.getElementById('targetAchievedInput').value||0),updatedDate=document.getElementById('targetUpdatedDateInput').value||new Date().toISOString().slice(0,10),targetSalesEmail=type==='outlet'?(document.getElementById('targetSalesInput')?.value||''):'',id=document.getElementById('targetEditId').value||`T-${Date.now()}`;if(!startMonth||!ownerId||!description||target<=0)return toast('Lengkapi periode, pemilik target, deskripsi target, dan target Rupiah.');if(![1,3,6,12].includes(durationMonths))return toast('Periode target hanya 1, 3, 6, atau 12 bulan.');if(type==='outlet'&&!targetSalesEmail)return toast('Pilih sales untuk target outlet agar muncul di Dashboard Sales.');const row={id,type,startMonth,month:startMonth,durationMonths,ownerId,targetSalesEmail,description,target,achieved:Math.max(0,achieved),updatedDate,updatedAt:new Date().toISOString()};const token=sessionToken();if(!navigator.onLine||!token)return toast('Target memerlukan koneksi internet agar tersimpan untuk semua Sales.');try{await targetRpc('app_admin_upsert_dashboard_target',{p_token:token,p_target:targetToRemote(row)});await pullTargetsFromServer({silent:false});fillTargetDurationFilter();await renderDashboardTargets({sync:false});await renderSettingsTargetList();await renderTargetManagerList();document.getElementById('targetEditId').value='';document.getElementById('targetAmountInput').value='';document.getElementById('targetAchievedInput').value='';document.getElementById('targetDescriptionInput').value='';toast('Target berhasil disimpan dan tersedia di Dashboard Sales');}catch(e){toast(`Gagal menyimpan target: ${e.message||'periksa SQL/permission Supabase'}`)}}
