...
function isSupervisorUser(user=currentUser){
 return Boolean(user&&(user.role==="supervisor"||(user.role==="sales"&&user.canSwitchAreaFreely===true)));
}
...
function showAreaAssignments(){if(currentUser?.role!=="admin")return;hide();document.getElementById("assignmentView").classList.remove("hidden");renderAreaAssignments();renderUserManagement();applyAreaAssignmentPanelState()}
function renderAreaAssignments(){
 const areas=allAreas(),sales=USERS.filter(u=>u.role==="sales"),map=getAreaAssignments(),host=document.getElementById("areaAssignmentMatrix");if(!host)return;
 host.innerHTML=`<table class="assignment-matrix"><thead><tr><th>Area</th>${sales.map(u=>`<th>${esc(u.name)}</th>`).join("")}</tr></thead><tbody>${areas.map(a=>{const encoded=encodeURIComponent(a);return `<tr><td><div class="area-name-cell"><strong>${esc(a)}</strong><button class="area-edit-btn" type="button" onclick="renameAreaName('${encoded}')" aria-label="Edit nama area ${esc(a)}">Edit</button></div></td>${sales.map(u=>`<td><input type="checkbox" ${((map[u.email]||[]).includes(a))?"checked":""} onchange="toggleAreaAssignment('${u.email}','${encoded}',this.checked)"></td>`).join("")}</tr>`}).join("")}</tbody></table>`;
 const summary=document.getElementById("areaAssignmentCollapseSummary");if(summary){const totalChecked=sales.reduce((n,u)=>n+(map[u.email]||[]).length,0);summary.textContent=`${areas.length} area · ${sales.length} sales · ${totalChecked} penugasan aktif`;}applyAreaAssignmentPanelState();
}
function toggleAreaAssignment(email,encodedArea,checked){const area=decodeURIComponent(encodedArea),map=getAreaAssignments(),set=new Set(map[email]||[]);checked?set.add(area):set.delete(area);map[email]=[...set];localStorage.setItem(AREA_ASSIGNMENT_KEY,JSON.stringify(map));syncSettingsToSupabase().catch(e=>console.error(e));toast("Penugasan area disimpan")}
...
async function login(){
 ...
 try{
  ...
  await pullCustomersFromSupabase({silent:true});
   if(currentUser?.role==="sales")ensureSalesAreaAssignment(currentUser.email);
