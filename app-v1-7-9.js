...
async function pullAreaAssignmentsFromSupabase({silent=true}={}){
 ...
 try{
  ...
  const map={};
  list.forEach(x=>{
   const email=String(x?.sales_email||x?.email||"").trim().toLowerCase();
   const area=String(x?.area||"").trim();
   if(email&&area)(map[email]||(map[email]=[])).push(area);
  });
  const email=String(currentUser.email||"").trim().toLowerCase();
  if(email){
   const existing=getAreaAssignments();
   existing[email]=[...new Set(map[email]||[])];
   localStorage.setItem(AREA_ASSIGNMENT_KEY,JSON.stringify(existing));
  }
  return true;
 }catch(e){
