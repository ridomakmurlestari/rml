...
function customerExportRows(rows){
 return rows.map(c=>({"Kode Outlet":c.code||"","Nama Outlet":c.name||"","Area":c.area||"","Pemilik":c.owner||c.pemilik||"","Sales":customerAssignedSales(c).map(e=>USERS.find(u=>u.email===e)?.name||e).join(", "),"Email Sales":customerAssignedSales(c).join(", "),"Aktif":c.isHidden?"Tidak":"Ya","Status Outlet":c.outletStatus||"normal"}));
}
...
function exportCustomersExcel(){
 ...
 if(window.XLSX){const ws=XLSX.utils.json_to_sheet(rows),wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,"Pelanggan");XLSX.writeFile(wb,filename+".xlsx")}else downloadCsv(rows,filename+".csv");
