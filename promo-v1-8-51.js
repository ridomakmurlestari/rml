...
(function(){
  ...
  function openDetail(email){const cats=cardsForOwner(email).filter(c=>c.items.length);if(!cats.length)return;const view=document.getElementById('promoCatalogView');if(!view)return;if(typeof hide==='function')hide();document.getElementById('promoCatalogOwner').textContent='Promo';document.getElementById('promoCatalogMonth').textContent=monthLabel(monthKey());document.getElementById('promoCatalogList').innerHTML=cats.map(c=>`<section class="promo-catalog-category"><div class="promo-catalog-category-head"><span>🎁</span><div><strong>${escP(c.name)}</strong><small>${c.items.length} barang promo</small></div></div><div class="promo-catalog-items">${c.items.map((x,i)=>{const it=normalizeItem(x);return `<div class="promo-catalog-item promo-catalog-item-text"><span>${i+1}</span><strong>${escP(it.name)}</strong></div>`}).join('')}</div></section>`).join('');view.classList.remove('hidden')}
