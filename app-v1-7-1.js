...
async function refreshDashboard(){
 ...
 if(!isAdmin){
   ...
   document.getElementById("salesStatsList").innerHTML=`<div class="sales-stat-card">
     ...
     ${isSupervisorUser()?`<button class="primary compact" onclick="openOutsideAreaOrder()">+ Order Luar Area</button>`:(area?`<button class="primary compact" onclick="showCustomers()">Lanjutkan Area ${esc(area)}</button>`:`<button class="primary compact" onclick="showDailyAreaSelection()">Pilih Area Hari Ini</button>`)}
   </div>`;
   return;
 }
...
async function showCustomers(){
 await refreshVisitCache();
 if(currentUser&&currentUser.role==="sales"&&!isSupervisorUser()&&!getDailyArea())return showDailyAreaSelection();
 hide();
 ...
 if(currentUser.role==="sales"&&!isSupervisorUser()){
