...
async function syncSettingsToSupabase(){const session=getSbSession();if(currentUser?.role!=='admin'||!navigator.onLine||!session?.session_token)return;const users=USERS.map(u=>({account_key:u.email,display_name:u.name,login_name:(u.loginName||u.name).trim().toLowerCase(),phone:u.phone,role:u.role,active:u.active!==false}));const map=getAreaAssignments();const assignments=[];for(const u of USERS.filter(x=>x.role==='sales'))for(const area of (map[u.email]||[]))assignments.push({sales_email:u.email,area});await rpc('app_admin_save_settings',{p_token:session.session_token,p_users:users,p_assignments:assignments})}
const APP_VERSION="0.10.7";
const USER_SETTINGS_KEY="rml_user_accounts_v1";
const DEFAULT_USERS=[
{email:"rini@rml.app",loginName:"rini",active:true,phone:"085668027045",name:"Rini",role:"sales",password:"085668027045",mustChangePassword:true},
{email:"lisna@rml.app",loginName:"lisna",active:true,phone:"085218600582",name:"Lisna",role:"sales",password:"085218600582",mustChangePassword:true},
{email:"septino@rml.app",loginName:"septino",active:true,phone:"08116946999",name:"Septino",role:"sales",password:"08116946999",mustChangePassword:true},
{email:"admin@rml.app",loginName:"admin",active:true,phone:"082284879722",name:"Admin",role:"admin",password:"082284879722",mustChangePassword:true}];
function loadUsers(){
 try{
...
async function saveUserAccount(index){
 ...
 const before={name:user.name,loginName:user.loginName||user.name.toLowerCase(),phone:user.phone,active:user.active!==false};
 try{
  if(btn){btn.disabled=true;btn.textContent="Menyimpan..."}
  await rpc('app_admin_update_user',{p_token:session.session_token,p_account_key:user.email,p_display_name:name,p_login_name:loginName,p_phone:phone,p_active:active});
  user.name=name;user.loginName=loginName;user.phone=phone;user.active=active;persistUsers();
