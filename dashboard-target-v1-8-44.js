...
function rewardHtml(x){if(!x.rewardType||x.rewardType==='none'||Number(x.rewardValue||0)<=0)return '';const earned=rewardEarnedAmount(x),full=rewardBaseAmount(x);return `<div class=\"target-reward-box\"><div><small>Reward</small><strong>${escTarget(rewardText(x))}</strong></div><div class=\"target-reward-full\"><small>Reward 100%</small><strong>${money(full)}</strong></div></div>`}
