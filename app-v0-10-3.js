...
async function syncSettingsToSupabase(){const session=getSbSession();if(currentUser?.role!=='admin'||!navigator.onLine||!session?.session_token)return;const users=USERS.map(u=>({account_key:u.email,display_name:u.name,phone:u.phone,role:u.role}));const map=getAreaAssignments();const assignments=[];for(const u of USERS.filter(x=>x.role==='sales'))for(const area of (map[u.email]||[]))assignments.push({sales_email:u.email,area});await rpc('app_admin_save_settings',{p_token:session.session_token,p_users:users,p_assignments:assignments})}
const APP_VERSION="0.10.3";
const USER_SETTINGS_KEY="rml_user_accounts_v1";
const DEFAULT_USERS=[
{email:"rini@rml.app",phone:"085668027045",name:"Rini",role:"sales",password:"085668027045",mustChangePassword:true},
{email:"lisna@rml.app",phone:"085218600582",name:"Lisna",role:"sales",password:"085218600582",mustChangePassword:true},
{email:"septino@rml.app",phone:"08116946999",name:"Septino",role:"sales",password:"08116946999",mustChangePassword:true},
{email:"admin@rml.app",phone:"082284879722",name:"Administrator",role:"admin",password:"082284879722",mustChangePassword:true}];
function loadUsers(){
 try{
  ...
  if(Array.isArray(saved)&&saved.length===DEFAULT_USERS.length){
   return DEFAULT_USERS.map(def=>{const old=saved.find(x=>x.email===def.email)||{};const merged={...def,...old};if(!old.password){merged.password=normalizePhone(old.phone||def.phone);merged.mustChangePassword=true}return merged});
  }
 }catch(e){}
...
function renderUserManagement(){
 ...
 host.innerHTML=USERS.map((u,index)=>`<div class="user-setting-card">
  ...
  <div class="user-setting-actions"><button class="primary compact" type="button" onclick="saveUserAccount(${index})">Simpan Akun</button><button class="secondary compact" type="button" onclick="resetUserPassword(${index})">Reset Password</button></div>
 </div>`).join("");
}
function saveUserAccount(index){
 ...
 const duplicateName=USERS.some((u,i)=>i!==index&&u.name.trim().toLowerCase()===name.toLowerCase());
 if(duplicateName)return toast("Nama login sudah digunakan akun lain");
 ...
 user.name=name;user.phone=phone;persistUsers();
 if(currentUser.email===user.email){currentUser={...user};sessionStorage.setItem("rml_user",JSON.stringify(currentUser));document.getElementById("userText").textContent=currentUser.name;}
 fillAreas();fillHistorySalesFilter();renderAreaAssignments();renderUserManagement();syncSettingsToSupabase().catch(e=>console.error(e));toast("Akun berhasil diperbarui");
}
...
async function saveForcedPassword(){
 ...
 if(old!==user.password)return toast("Password lama salah");
 ...
 if(next===user.password)return toast("Password baru harus berbeda");
 try{await sbUpdatePassword(old,next)}catch(e){return toast(`Gagal mengubah password: ${e.message}`)}
...
async function login(){
 ...
 try{
  ...
  currentUser={...local,email:p.account_key,phone:p.phone||local.phone||"",name:p.display_name,role:p.role,mustChangePassword:!!p.must_change_password};
...
async function logout(){
 if(currentUser?.role==="sales"){
   ...
   if(!area)return completeLogout();
   const pending=getPendingOutletsForArea(area);
   return openUnvisitedReasonModal({type:"logout",area,pending});
 }
 completeLogout();
}
function completeLogout(){sessionStorage.removeItem("rml_user");localStorage.removeItem("rml_cached_user");setSbSession(null);location.reload()}
function openApp(){
 initializeData();
 if(navigator.onLine)setTimeout(()=>syncPendingVisits(),800);
 document.getElementById("loginPage").classList.add("hidden");
 document.getElementById("appPage").classList.remove("hidden");
 document.getElementById("roleText").textContent=currentUser.role==="admin"?"Administrator":"Sales";
 document.getElementById("userText").textContent=currentUser.name;
 document.getElementById("finishBtn").textContent=currentUser.role==="sales"?"Selesai":"Keluar";
...
function openUnvisitedReasonModal(action){
 ...
 document.getElementById("saveFinishAreaBtn").textContent=action.pending.length?"Simpan Alasan & Selesai":"Akhiri Hari Ini";
...
async function saveUnvisitedReasons(){
 ...
 if(action.type==="logout")return setTimeout(completeLogout,400);
...
function renderCustomers(){
 ...
 let base=currentUser.role==="sales"
   ?all.filter(x=>!x.isHidden&&isAreaAssigned(currentUser.email,x.area)&&x.area===getDailyArea())
   :all;
 const data=base
   .filter(x=>(!a||x.area===a)&&(!s||(s==="hidden"?x.isHidden:!x.isHidden))&&(!os||(x.outletStatus||"normal")===os)&&(`${x.code} ${x.name}`.toLowerCase().includes(q)))
   .sort((x,y)=>{
     if(currentUser.role==="sales"){
       const xv=Boolean(todayVisitForOutlet(x.no));
       const yv=Boolean(todayVisitForOutlet(y.no));
       if(xv!==yv)return xv?1:-1;
     }
     return compareCustomerCode(x,y);
   });
 document.getElementById("customerCount").textContent=base.length;
 document.getElementById("customerList").innerHTML=data.map(x=>`
 <div class="customer-card ${x.isHidden?"hidden-customer":""} ${reminderClass(x.no)} ${currentUser.role==="sales"?(todayVisitForOutlet(x.no)?"today-complete":"today-pending"):""}" onclick="selectCustomer(${x.no})">
  <div>
   <div class="customer-code">${esc(x.code||"TANPA KODE")}</div>
   <h4>${esc(x.name)}</h4>
   <div class="customer-sub">${x.isHidden?"Hidden dari sales":"Aktif"}${currentUser.role==="admin"?" • "+esc(getSalesName(x.assignedSalesEmail)):""}</div>
   <div class="status-line">
     <span class="badge ${getOutletStatusClass(x.outletStatus)}">${esc(getOutletStatusLabel(x.outletStatus))}</span>
     ${x.outletStatus==="dueSoon"&&x.dueDate?`<span class="badge">${esc(formatDate(x.dueDate))}</span>`:""}
   </div>
   ${currentUser.role==="sales"&&isOutletBeingVisited(x)?`<div class="customer-visit-state visiting">● Sedang Dikunjungi</div>`:""}
   <div class="last-visit-label">${lastVisitText(x.no)}</div>
   ${currentUser.role==="sales"?`<div class="today-visit-state ${todayVisitForOutlet(x.no)?"done":"pending"}">${todayVisitForOutlet(x.no)?`✓ ${esc(todayVisitForOutlet(x.no).status)}`:"○ Belum dikunjungi hari ini"}</div>`:""}
  </div>
  <div style="display:grid;gap:6px;justify-items:end">
   <span class="badge">${esc(x.area)}</span>
   ${currentUser.role==="admin"?`<span class="badge ${x.isHidden?"status-hidden":"status-active"}">${x.isHidden?"Hidden":"Aktif"}</span>`:""}
  </div>
 </div>`).join("");
...
function selectCustomer(no){
 ...
 if(currentUser.role==="sales"&&selected.assignedSalesEmail!==currentUser.email&&selected.assignedSalesEmail!=="__ALL__")
