...
window.addEventListener("focus",()=>{scheduleAutoSync(150);if(currentUser&&navigator.onLine)pullCustomersFromSupabase({silent:true}).then(()=>{if(!document.getElementById("customersView")?.classList.contains("hidden"))renderCustomers()}).catch(()=>{})});
document.addEventListener("visibilitychange",()=>{if(document.visibilityState==="visible"){scheduleAutoSync(150);if(currentUser&&navigator.onLine)pullCustomersFromSupabase({silent:true}).then(()=>{if(!document.getElementById("customersView")?.classList.contains("hidden"))renderCustomers()}).catch(()=>{})}});
