...
const REMOTE_VISIT_INITIAL_LIMIT=500;
...
async function syncPendingVisits(){
 ...
 try{
  ...
  for(let i=0;i<pending.length;i+=batchSize){
   ...
   await Promise.all(batch.map(async v=>{await rpc('app_upsert_visit',{p_token:getSbSession().session_token,p_visit:visitToRemote(v)});await idbPut(STORE_VISITS,{...v,syncStatus:"synced"})}));
  }
